...
range(GISAID_sel$date) # "2020-01-20" "2021-06-03"
...
muller_japan_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN JAPAN\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_japan_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\japan_muller plots_raw data.png"), width=8, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\japan_muller plots_raw data.pdf"), width=8, height=6)
...
library(splines)
set.seed(1)
fit1_japan_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_japan_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_japan_multi, fit2_japan_multi) 
#             df      BIC
# fit1_japan_multi 20 91193.91
# fit2_japan_multi 30 79534.42

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrjapan = emtrends(fit2_japan_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_japan = data.frame(confint(emtrjapan, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrjapan$contrasts)$p.value)
delta_r_japan
# contrast     estimate           SE df    asymp.LCL     asymp.UCL      p.value
# 1      B.1.1 - B.1.1.7 -0.061421512 0.001353966 NA -0.06407524 -0.05876779 4.152234e-14
# 2  B.1.1.214 - B.1.1.7 -0.106042208 0.001266538 NA -0.10852458 -0.10355984 4.152234e-14
# 3  B.1.1.284 - B.1.1.7 -0.126419825 0.001529651 NA -0.12941789 -0.12342176 4.152234e-14
# 4  B.1.1.519 - B.1.1.7 -0.292026471 0.075504656 NA -0.44001288 -0.14404006 4.710421e-03
# 5    B.1.351 - B.1.1.7 -0.128057457 0.011626904 NA -0.15084577 -0.10526914 4.530976e-11
# 6        P.1 - B.1.1.7 -0.117124016 0.006572287 NA -0.13000546 -0.10424257 4.163336e-14
# 7        R.1 - B.1.1.7 -0.039580995 0.001054173 NA -0.04164714 -0.03751485 4.152234e-14
# 8  B.1.617.1 - B.1.1.7 -0.002324826 0.027668547 NA -0.05655418  0.05190453 9.999892e-01
# 9  B.1.617.2 - B.1.1.7  0.168280867 0.023678657 NA  0.12187155  0.21469018 6.403575e-07
# 10     other - B.1.1.7 -0.046043114 0.001432896 NA -0.04885154 -0.04323469 4.152234e-14


# fitted prop of different LINEAGES in the japan today
# 99% [97%-100%] now estimated to be B.1.617.2 across all regions
multinom_preds_today_avg = data.frame(emmeans(fit2_japan_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1    B.1.1.7 6.903157e-02 7.025720e-02 NA -6.867001e-02 2.067332e-01
# 2      B.1.1 2.126395e-05 2.182687e-05 NA -2.151593e-05 6.404383e-05
# 3  B.1.1.214 2.054821e-06 2.104957e-06 NA -2.070820e-06 6.180462e-06
# 4  B.1.1.284 1.116582e-08 1.154696e-08 NA -1.146579e-08 3.379744e-08
# 5  B.1.1.519 5.332957e-21 6.675742e-20 NA -1.255092e-19 1.361751e-19
# 6    B.1.351 3.341889e-09 5.492508e-09 NA -7.423229e-09 1.410701e-08
# 7        P.1 1.577067e-07 1.847532e-07 NA -2.044029e-07 5.198164e-07
# 8        R.1 1.915952e-03 1.954878e-03 NA -1.915539e-03 5.747442e-03
# 9  B.1.617.1 3.820397e-05 7.987839e-05 NA -1.183548e-04 1.947627e-04
# 10 B.1.617.2 9.289144e-01 7.234728e-02 NA  7.871163e-01 1.070712e+00
# 11     other 7.640050e-05 7.842018e-05 NA -7.730022e-05 2.301012e-04

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9309684 0.7849813 1.0769556 
...
fit_japan_multi_preds = data.frame(predgrid, as.data.frame(predict(fit2_japan_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_japan_multi_preds = gather(fit_japan_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_japan_multi_preds$collection_date = as.Date(fit_japan_multi_preds$DATE_NUM, origin="1970-01-01")
fit_japan_multi_preds$LINEAGE2 = factor(fit_japan_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 

muller_japan_mfit = ggplot(data=fit_japan_multi_preds, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN JAPAN\n(GISAID data, multinomial fit)")
muller_japan_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\japan_muller plots_multinom fit.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\japan_muller plots_multinom fit.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_japan_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_japan_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\japan_muller plots multipanel_multinom fit.png"), width=10, height=10)
ggsave(file=paste0(".\\plots\\",plotdir,"\\japan_muller plots multipanel_multinom fit.pdf"), width=10, height=10)
