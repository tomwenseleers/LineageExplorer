...
range(GISAID_sel$date) # "2020-01-24" "2021-06-11"
...
muller_australia_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
...
library(splines)
set.seed(1)
fit1_australia_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_australia_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_australia_multi, fit2_australia_multi) 
#             df      BIC
# fit1_australia_multi 16 3395.412
# fit2_australia_multi 24 3320.458

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtraustralia = emtrends(fit1_australia_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_australia = data.frame(confint(emtraustralia, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtraustralia$contrasts)$p.value)
delta_r_australia
# contrast     estimate           SE df    asymp.LCL     asymp.UCL      p.value
# 1       B.1 - B.1.1.7 -0.020284802 0.003161198 NA -0.026480636 -0.014088968 6.080065e-05
# 2 B.1.1.519 - B.1.1.7  0.031923487 0.015502070 NA  0.001539989  0.062306985 2.641317e-01
# 3   B.1.351 - B.1.1.7 -0.003816971 0.002816517 NA -0.009337243  0.001703300 6.374561e-01
# 4       D.2 - B.1.1.7 -0.251580364 0.126366588 NA -0.499254324 -0.003906404 2.929135e-01
# 5       P.1 - B.1.1.7  0.009165435 0.009613156 NA -0.009676004  0.028006874 8.506341e-01
# 6 B.1.617.1 - B.1.1.7  0.057181006 0.005933855 NA  0.045550864  0.068811148 3.372488e-07
# 7 B.1.617.2 - B.1.1.7  0.043206874 0.004254321 NA  0.034868558  0.051545190 1.630998e-07
# 8     other - B.1.1.7 -0.022184357 0.001432457 NA -0.024991920 -0.019376793 3.474664e-10


# fitted prop of different LINEAGES in the australia today
multinom_preds_today_avg = data.frame(emmeans(fit2_australia_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1   B.1.1.7 9.849444e-02 3.063686e-02 NA  3.844729e-02 1.585416e-01
# 2       B.1 5.422173e-02 2.894058e-02 NA -2.500755e-03 1.109442e-01
# 3 B.1.1.519 2.604772e-05 1.266181e-04 NA -2.221192e-04 2.742146e-04
# 4   B.1.351 2.505473e-02 1.392067e-02 NA -2.229293e-03 5.233875e-02
# 5       D.2 6.060988e-33 2.820842e-32 NA -4.922649e-32 6.134847e-32
# 6       P.1 1.795960e-05 7.996979e-05 NA -1.387783e-04 1.746975e-04
# 7 B.1.617.1 6.193942e-01 7.939286e-02 NA  4.637870e-01 7.750013e-01
# 8 B.1.617.2 1.302946e-01 4.607916e-02 NA  3.998111e-02 2.206081e-01
# 9     other 7.249632e-02 2.483466e-02 NA  2.382128e-02 1.211714e-01

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9015056 0.5224985 1.2805127 
...
fit_australia_multi_preds = data.frame(predgrid, as.data.frame(predict(fit1_australia_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_australia_multi_preds = gather(fit_australia_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_australia_multi_preds$collection_date = as.Date(fit_australia_multi_preds$DATE_NUM, origin="1970-01-01")
fit_australia_multi_preds$LINEAGE2 = factor(fit_australia_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 
...
library(ggpubr)
ggarrange(muller_australia_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
