...
# 454      802     4679     1341       96      728     1603      169 
...
levels_LINEAGE1 = c("B.1.1.7","B.1.177+",
...
# "B.1.1.7"   "B.1.177+"  "B.1.351"   "B.1.617.1" "B.1.617.2" "other"     "P.1" 
levels_LINEAGE2 = c("B.1.1.7","B.1.177+",
...
# aggregate by day to identify days on which INSA performed baseline surveillance (days with a lot of sequences)
...
GISAID_sel = GISAID_sel[GISAID_sel$total_sequenced_on_that_day>20,] # dates on which baseline surveillance was performed
...
muller_portugal_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01")),
                     ...
                     limits=as.Date(c("2021-02-01",NA)), 
                     expand=c(0,0)) +
...
# df      BIC
# fit1_portugal_multi 12 8176.180
# fit2_portugal_multi 18 8043.871
...
# 1 (B.1.177+) - B.1.1.7 -0.05581888 0.025483171 NA -0.10576498 -0.005872785 1.741393e-01
# 2    B.1.351 - B.1.1.7 -0.03277813 0.014848702 NA -0.06188106 -0.003675212 1.690578e-01
# 3        P.1 - B.1.1.7 -0.00632620 0.007622421 NA -0.02126587  0.008613471 8.598001e-01
# 4  B.1.617.1 - B.1.1.7 -0.98672776 0.599795722 NA -2.16230577  0.188850256 4.044027e-01
# 5  B.1.617.2 - B.1.1.7  0.09301972 0.011259119 NA  0.07095225  0.115087184 8.757008e-07
# 6      other - B.1.1.7  0.04823980 0.004840895 NA  0.03875182  0.057727779 5.382958e-08
...
# 1   B.1.1.7 7.390771e-02 1.851381e-02 NA  3.762131e-02 1.101941e-01
# 2  B.1.177+ 7.475671e-06 1.346004e-05 NA -1.890553e-05 3.385687e-05
# 3   B.1.351 1.627962e-04 1.493915e-04 NA -1.300057e-04 4.555982e-04
# 4       P.1 2.261638e-03 1.048686e-03 NA  2.062506e-04 4.317025e-03
# 5 B.1.617.1 4.057948e-24 1.263736e-22 NA -2.436298e-22 2.517457e-22
# 6 B.1.617.2 8.943854e-01 2.620214e-02 NA  8.430302e-01 9.457407e-01
# 7     other 2.927494e-02 9.189352e-03 NA  1.126414e-02 4.728574e-02

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9260923 0.8543517 0.9978329 
...
library(ggpubr)
ggarrange(muller_portugal_raw2 + coord_cartesian(xlim=c(as.Date("2021-02-01"),as.Date(date.to, origin="1970-01-01")))+
...
fit_portugal_multi_preds_withCI = data.frame(emmeans(fit2_portugal_multi,
                                                        ~ LINEAGE2,
                                                        by=c("DATE_NUM"),
...
fit_portugal_multi_preds2 = fit_portugal_multi_preds_withCI
...
plot_portugal_mfit_logit = qplot(data=fit_portugal_multi_preds2, x=collection_date, y=prob, geom="blank") +
  ...
  ylab("Share (%)") +
  theme_hc() + xlab("") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN PORTUGAL\n(baseline surveillance GISAID data, multinomial fit)") +
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01")),
                     labels=substring(months(as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01"))),1,1),
                     limits=as.Date(c("2021-02-01",NA)), expand=c(0,0)) +
  ...
  coord_cartesian(xlim=c(as.Date("2021-02-01"),as.Date(date.to, origin="1970-01-01")), ylim=c(0.001, 0.9901), expand=c(0,0))
...
cases_tot = as.data.frame(get_national_data(countries = "Portugal"))
...
fit_portugal_multi_preds_withCI$totcases = cases_tot$cases_new[match(round(fit_portugal_multi_preds_withCI$DATE_NUM),cases_tot$DATE_NUM)]
fit_portugal_multi_preds_withCI$cases = fit_portugal_multi_preds_withCI$totcases * fit_portugal_multi_preds_withCI$prob
fit_portugal_multi_preds_withCI$cases[fit_portugal_multi_preds_withCI$cases<=0.001] = NA
...
fit_portugal_multi_preds_withCI$smoothed_totcases = cases_emmeans$rate[match(fit_portugal_multi_preds_withCI$DATE_NUM,cases_emmeans$DATE_NUM)]
fit_portugal_multi_preds_withCI$smoothed_cases = fit_portugal_multi_preds_withCI$smoothed_totcases * fit_portugal_multi_preds_withCI$prob
fit_portugal_multi_preds_withCI$smoothed_cases[fit_portugal_multi_preds_withCI$smoothed_cases<=0.001] = NA

ggplot(data=fit_portugal_multi_preds_withCI[fit_portugal_multi_preds_withCI$collection_date>=as.Date("2021-02-18"),], 
  ...
  ggtitle("NEW CONFIRMED SARS-CoV2 CASES PER DAY BY VARIANT\nIN PORTUGAL\n(case data & multinomial fit to baseline surveillance GISAID data)") +
  scale_fill_manual("variant", values=lineage_cols2) +
  scale_colour_manual("variant", values=lineage_cols2) +
...
ggplot(data=fit_portugal_multi_preds_withCI[fit_portugal_multi_preds_withCI$collection_date>=as.Date("2021-02-18")&
                                              fit_portugal_multi_preds_withCI$collection_date<=max(cases_tot$date),], 
...
qplot(data=avg_r_cases, x=DATE-7, y=Re, ymin=Re_LOWER, ymax=Re_UPPER, geom="ribbon", alpha=I(0.5), fill=I("steelblue")) +
  ...
  ggtitle("Re IN PORTUGAL AT MOMENT OF INFECTION BASED ON NEW CASES") +
  # labs(tag = tag) +
...
above_avg_r_variants0 = do.call(rbind, lapply(seq(date.from,
                                                  date.to), 
                                              function (d) { 
                                                wt = as.data.frame(emmeans(fit2_portugal_multi, ~ LINEAGE2 , at=list(DATE_NUM=d), type="response"))$prob   # important: these should sum to 1
                                                ...
                                                EMT = emtrends(fit2_portugal_multi,  ~ LINEAGE2 , by=c("DATE_NUM"),
                                                               var="DATE_NUM", mode="latent",
                                                               at=list(DATE_NUM=d))
...
above_avg_r_variants$prob = fit_portugal_multi_preds_withCI$prob[match(interaction(above_avg_r_variants$DATE_NUM,
                                                                      above_avg_r_variants$variant),
                                                          interaction(fit_portugal_multi_preds_withCI$DATE_NUM,
                                                                      fit_portugal_multi_preds_withCI$LINEAGE2))]
...
qplot(data=above_avg_r_variants2[!((above_avg_r_variants2$variant %in% c("other"))|above_avg_r_variants2$collection_date>max(cases_tot$DATE)),], 
  ...
  geom_hline(yintercept=1, colour=I("red")) +
  ggtitle("Re VALUES OF SARS-CoV2 VARIANTS IN PORTUGAL\nAT MOMENT OF INFECTION\n(based on case data & multinomial fit to\nbaseline surveillance GISAID data)") +
  # labs(tag = tag) +
  # theme(plot.margin = margin(t = 20, r = 10, b = 20, l = 0)) +
  theme(plot.tag.position = "bottomright",
        plot.tag = element_text(vjust = 1, hjust = 1, size=8)) +
