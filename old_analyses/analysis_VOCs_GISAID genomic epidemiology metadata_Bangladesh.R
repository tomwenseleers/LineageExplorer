...
muller_bangladesh_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN BANGLADESH\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_bangladesh_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\bangladesh_muller plots_raw data.png"), width=8, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\bangladesh_muller plots_raw data.pdf"), width=8, height=6)
...
library(splines)
set.seed(1)
fit1_bangladesh_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_bangladesh_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_bangladesh_multi, fit2_bangladesh_multi) 
#             df      BIC
# fit1_bangladesh_multi 22 2064.954
# fit2_bangladesh_multi 33 2069.697

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrbangladesh = emtrends(fit1_bangladesh_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_bangladesh = data.frame(confint(emtrbangladesh, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrbangladesh$contrasts)$p.value)
delta_r_bangladesh
# contrast     estimate           SE df    asymp.LCL     asymp.UCL      p.value
# 1        B.1 - B.1.1.7 -0.015760582 0.005588905 NA  -0.02671463 -0.0048065282 7.566629e-02
# 2      B.1.1 - B.1.1.7 -0.025255533 0.008620075 NA  -0.04215057 -0.0083604967 6.030154e-02
# 3   B.1.1.25 - B.1.1.7 -0.036967264 0.003958733 NA  -0.04472624 -0.0292082900 4.305119e-08
# 4  B.1.1.318 - B.1.1.7  0.046776477 0.010112530 NA   0.02695628  0.0665966716 1.239353e-03
# 5    B.1.351 - B.1.1.7  0.039217075 0.008375764 NA   0.02280088  0.0556332701 1.083073e-03
# 6  B.1.351.3 - B.1.1.7  0.035065165 0.004511982 NA   0.02622184  0.0439084881 9.789453e-07
# 7  (B.1.36+) - B.1.1.7 -0.043928528 0.009918480 NA  -0.06336839 -0.0244886651 1.978623e-03
# 8        P.1 - B.1.1.7 -0.008822198 0.024879085 NA  -0.05758431  0.0399399134 9.975844e-01
# 9  B.1.617.1 - B.1.1.7 -0.018648275 7.173050533 NA -14.07756898 14.0402724288 1.000000e+00
# 10 B.1.617.2 - B.1.1.7  0.161924548 0.022826249 NA   0.11718592  0.2066631736 4.171027e-06
# 11     other - B.1.1.7 -0.008863306 0.005008406 NA  -0.01867960  0.0009529889 4.547713e-01


# fitted prop of different LINEAGES in the bangladesh today
multinom_preds_today_avg = data.frame(emmeans(fit1_bangladesh_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1    B.1.1.7 2.753713e-04 2.431541e-04 NA -2.012019e-04 7.519445e-04
# 2        B.1 1.272144e-05 1.359463e-05 NA -1.392355e-05 3.936644e-05
# 3      B.1.1 1.139971e-06 1.732180e-06 NA -2.255038e-06 4.534981e-06
# 4   B.1.1.25 4.756038e-06 4.507721e-06 NA -4.078933e-06 1.359101e-05
# 5  B.1.1.318 3.710757e-03 3.671173e-03 NA -3.484609e-03 1.090612e-02
# 6    B.1.351 3.234426e-03 3.021596e-03 NA -2.687794e-03 9.156645e-03
# 7  B.1.351.3 2.600578e-02 1.995600e-02 NA -1.310727e-02 6.511882e-02
# 8    B.1.36+ 5.747289e-08 1.096374e-07 NA -1.574125e-07 2.723582e-07
# 9        P.1 1.187656e-06 3.895864e-06 NA -6.448098e-06 8.823410e-06
# 10 B.1.617.1 3.573058e-12 3.724473e-09 NA -7.296260e-09 7.303407e-09
# 11 B.1.617.2 9.667089e-01 2.538959e-02 NA  9.169462e-01 1.016472e+00
# 12     other 4.490368e-05 4.393845e-05 NA -4.121409e-05 1.310215e-04

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9997246 0.8975985 1.1018508 
...
fit_bangladesh_multi_preds = data.frame(predgrid, as.data.frame(predict(fit1_bangladesh_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_bangladesh_multi_preds = gather(fit_bangladesh_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_bangladesh_multi_preds$collection_date = as.Date(fit_bangladesh_multi_preds$DATE_NUM, origin="1970-01-01")
fit_bangladesh_multi_preds$LINEAGE2 = factor(fit_bangladesh_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 

muller_bangladesh_mfit = ggplot(data=fit_bangladesh_multi_preds, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN BANGLADESH\n(GISAID data, multinomial fit)")
muller_bangladesh_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\bangladesh_muller plots_multinom fit.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\bangladesh_muller plots_multinom fit.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_bangladesh_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_bangladesh_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\bangladesh_muller plots multipanel_multinom fit.png"), width=10, height=10)
ggsave(file=paste0(".\\plots\\",plotdir,"\\bangladesh_muller plots multipanel_multinom fit.pdf"), width=10, height=10)
