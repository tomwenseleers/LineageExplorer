...
muller_thailand_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
...
library(splines)
set.seed(1)
fit1_thailand_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_thailand_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_thailand_multi, fit2_thailand_multi) 
#             df      BIC
# fit1_thailand_multi 14 2199.812
# fit2_thailand_multi 21 1981.823

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrthailand = emtrends(fit2_thailand_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_thailand = data.frame(confint(emtrthailand, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrthailand$contrasts)$p.value)
delta_r_thailand
# contrast     estimate           SE df    asymp.LCL     asymp.UCL      p.value
# 1       A.6 - B.1.1.7 -7.39283000 1.087719e+00 NA -9.5247195131 -5.26094048 6.669265e-06
# 2       B.1 - B.1.1.7 -0.04196883 1.282189e-02 NA -0.0670992744 -0.01683839 2.074680e-02
# 3   B.1.351 - B.1.1.7  0.05965648 3.027124e-02 NA  0.0003259514  0.11898702 2.691903e-01
# 4 (B.1.36+) - B.1.1.7 -0.10783988 1.467053e-02 NA -0.1365935983 -0.07908616 2.062756e-06
# 5 B.1.617.1 - B.1.1.7  0.02794405 1.406606e-09 NA  0.0279440469  0.02794405 3.741452e-14
# 6 B.1.617.2 - B.1.1.7  0.18039670 2.092808e-02 NA  0.1393784286  0.22141498 1.631665e-07
# 7     other - B.1.1.7 -0.03570185 1.190446e-02 NA -0.0590341612 -0.01236954 3.775935e-02


# fitted prop of different LINEAGES in the thailand today
# 99% [97%-100%] now estimated to be B.1.617.2 across all regions
multinom_preds_today_avg = data.frame(emmeans(fit2_thailand_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1   B.1.1.7 8.198464e-03 5.947381e-03 NA -3.458190e-03 1.985512e-02
# 2       A.6 0.000000e+00 0.000000e+00 NA  0.000000e+00 0.000000e+00
# 3       B.1 5.092492e-06 6.065819e-06 NA -6.796295e-06 1.698128e-05
# 4   B.1.351 2.671163e-03 3.556271e-03 NA -4.299001e-03 9.641327e-03
# 5   B.1.36+ 5.086955e-07 6.225474e-07 NA -7.114749e-07 1.728866e-06
# 6 B.1.617.1 1.555789e-13 1.128611e-13 NA -6.562467e-14 3.767826e-13
# 7 B.1.617.2 9.890768e-01 8.246020e-03 NA  9.729149e-01 1.005239e+00
# 8     other 4.798542e-05 4.634044e-05 NA -4.284018e-05 1.388110e-04

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9918015 0.9685655 1.0150375 
...
library(tidyr)
library(tidyselect)
fit_thailand_multi_preds = gather(fit_thailand_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_thailand_multi_preds$collection_date = as.Date(fit_thailand_multi_preds$DATE_NUM, origin="1970-01-01")
fit_thailand_multi_preds$LINEAGE2 = factor(fit_thailand_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 
...
library(ggpubr)
ggarrange(muller_thailand_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
