...
muller_sinagpore_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN SINGAPORE\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_sinagpore_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\sinagpore_muller plots_raw data.png"), width=8, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\sinagpore_muller plots_raw data.pdf"), width=8, height=6)
...
library(splines)
set.seed(1)
fit1_sinagpore_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_sinagpore_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_sinagpore_multi, fit2_sinagpore_multi) 
#             df      BIC
# fit1_sinagpore_multi 14 957.7659
# fit2_sinagpore_multi 21 896.3350

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrsinagpore = emtrends(fit1_sinagpore_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_sinagpore = data.frame(confint(emtrsinagpore, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrsinagpore$contrasts)$p.value)
delta_r_sinagpore
# contrast     estimate           SE df    asymp.LCL     asymp.UCL      p.value
# 1         B - B.1.1.7 -0.080428605 4.518764200 NA -8.937043691 8.776186482 9.999995e-01
# 2   B.1.351 - B.1.1.7  0.044767780 0.010474083 NA  0.024238955 0.065296604 4.476099e-03
# 3   B.1.524 - B.1.1.7 -0.004536544 0.005355173 NA -0.015032490 0.005959401 8.752224e-01
# 4     B.6.6 - B.1.1.7 -0.080428605 4.518764201 NA -8.937043693 8.776186483 9.999995e-01
# 5 B.1.617.1 - B.1.1.7  0.023972348 0.009725696 NA  0.004910334 0.043034363 1.298940e-01
# 6 B.1.617.2 - B.1.1.7  0.142082569 0.013091088 NA  0.116424508 0.167740629 2.141800e-07
# 7     other - B.1.1.7 -0.004852228 0.005148706 NA -0.014943507 0.005239051 8.342121e-01


# fitted prop of different LINEAGES in the sinagpore today
multinom_preds_today_avg = data.frame(emmeans(fit1_sinagpore_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1   B.1.1.7 2.486839e-05 2.071574e-05 NA -1.573372e-05 6.547050e-05
# 2         B 1.299219e-16 1.291999e-13 NA -2.530973e-13 2.533571e-13
# 3   B.1.351 1.250501e-03 7.316515e-04 NA -1.835091e-04 2.684512e-03
# 4   B.1.524 5.345850e-05 3.709143e-05 NA -1.923936e-05 1.261564e-04
# 5     B.6.6 1.299219e-16 1.291999e-13 NA -2.530973e-13 2.533571e-13
# 6 B.1.617.1 1.601049e-04 1.257129e-04 NA -8.628774e-05 4.064976e-04
# 7 B.1.617.2 9.984290e-01 8.497413e-04 NA  9.967635e-01 1.000094e+00
# 8     other 8.210861e-05 5.475393e-05 NA -2.520711e-05 1.894243e-04

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9999751 0.9964493 1.0035010
...
fit_sinagpore_multi_preds = data.frame(predgrid, as.data.frame(predict(fit1_sinagpore_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_sinagpore_multi_preds = gather(fit_sinagpore_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_sinagpore_multi_preds$collection_date = as.Date(fit_sinagpore_multi_preds$DATE_NUM, origin="1970-01-01")
fit_sinagpore_multi_preds$LINEAGE2 = factor(fit_sinagpore_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 

muller_sinagpore_mfit = ggplot(data=fit_sinagpore_multi_preds, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN SINGAPORE\n(GISAID data, multinomial fit)")
muller_sinagpore_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\sinagpore_muller plots_multinom fit.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\sinagpore_muller plots_multinom fit.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_sinagpore_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_sinagpore_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\sinagpore_muller plots multipanel_multinom fit.png"), width=10, height=10)
ggsave(file=paste0(".\\plots\\",plotdir,"\\sinagpore_muller plots multipanel_multinom fit.pdf"), width=10, height=10)
