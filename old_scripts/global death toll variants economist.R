...
economist_excessdeaths_bycountry$date = as.Date(economist_excessdeaths_bycountry$date)
data_BEL = economist_excessdeaths_bycountry[economist_excessdeaths_bycountry$iso3c=="BEL",]
with(data_BEL,
     plot(x=date,
          y=daily_covid_deaths, pch=16, type="l", col="red",
          ylab="Daily confirmed Covid deaths (red) &\nest excess mortality (blue)"))
with(data_BEL,
     lines(x=date,
          y=estimated_daily_excess_deaths, pch=16, type="l", col="blue"))
sum(data_BEL$daily_covid_deaths)
sum(data_BEL$estimated_daily_excess_deaths)
