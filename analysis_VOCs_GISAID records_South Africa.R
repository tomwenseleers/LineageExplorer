...
# last update 24 JULY 2022
  ...
  sel_reference_VOC = "Omicron (BA.4)"
  levels_VARIANTS = c(sel_reference_VOC, "Beta", "Alpha", "Other", "Delta", "Omicron (BA.1)", "Omicron (BA.2)", "Omicron (BA.3)", "Omicron (BA.5)")
...
d1 = read_tsv(file(".//data//GISAID//South Africa//gisaid_hcov-19_2022_07_24_21_SouthAfrica_subm_2020.tsv"), col_types = cols(.default = "c")) 
d2 = read_tsv(file(".//data//GISAID//South Africa//gisaid_hcov-19_2022_07_24_21_SouthAfrica_subm_jan_2021_july_2021.tsv"), col_types = cols(.default = "c")) 
d3 = read_tsv(file(".//data//GISAID//South Africa//gisaid_hcov-19_2022_07_24_21_SouthAfrica_subm_aug_2021_oct_2021.tsv"), col_types = cols(.default = "c")) 
d4 = read_tsv(file(".//data//GISAID//South Africa//gisaid_hcov-19_2022_07_24_21_SouthAfrica_subm_nov_2021_dec_2021.tsv"), col_types = cols(.default = "c")) 
d5 = read_tsv(file(".//data//GISAID//South Africa//gisaid_hcov-19_2022_07_24_21_SouthAfrica_subm_jan_2022_march_2022.tsv"), col_types = cols(.default = "c")) 
d6 = read_tsv(file(".//data//GISAID//South Africa//gisaid_hcov-19_2022_07_24_21_SouthAfrica_subm_apr_2022_july_24_2022.tsv"), col_types = cols(.default = "c")) 
...
library(lubridate)
GISAID$date = as.Date(fast_strptime(GISAID$date, "%Y-%m-%d"))
GISAID = GISAID[!is.na(GISAID$date),]
GISAID = GISAID[GISAID$host=="Human",]
GISAID = GISAID[GISAID$date>=as.Date("2020-01-01"),]
range(GISAID$date) # "2020-03-06" "2022-07-16"
...
GISAID$floor_date = as.Date(as.character(cut(GISAID$date, "week")))+0 # start of week
...
GISAID$province = toupper(loc[,3])
GISAID = GISAID[GISAID$country=="South Africa"&
                  GISAID$province!="South Africa",]

GISAID$province = factor(GISAID$province)
GISAID$province = factor(GISAID$province, levels=levels(GISAID$province),
                                          labels=c("Western Cape", "Western Cape", "Eastern Cape", "Free State",
...
GISAID_sel = GISAID
nrow(GISAID_sel) # 43123
sum(GISAID_sel$variant==sel_target_VOC) # 1399
table(GISAID_sel$variant)
range(GISAID_sel$date) # "2020-03-06" "2022-07-14"
...
emtrsouthafrica_province_avg_pairw = emtrends(fit2_southafrica_province_multi, revpairwise ~ variant,  
                                        var="date_num",  mode="latent",
                                        at=list(date_num=seq(today_num, today_num, by=1)),
                                        adjust="none", df=NA)
...
date.to = today_num+extrapolate # max(GISAID_sel$date_num)
...
plot_southafrica_mfit_logit = qplot(data=fit_SA_multi_pr, x=date, y=prob, geom="blank") +
  ...
  coord_cartesian(xlim=c(as.Date("2020-03-01"),NA), ylim=c(0.001, 0.9901), expand=0)
...
plot_southafrica_byprovince_mfit_logit = qplot(data=fit_southafrica_multi_preds_byprovince2, x=date, y=prob, geom="blank") +
  ...
  theme(legend.position = "right") +
  xlab("Collection date")+
  coord_cartesian(xlim=c(as.Date("2021-01-01"),as.Date(date.to, origin="1970-01-01")), ylim=c(0.001, 0.9901), expand=0)
...
plot_southafrica_mfit_logit = qplot(data=fit_southafrica_multi_preds2, x=date, y=prob, geom="blank") +
  ...
  theme(legend.position = "right") +
  xlab("Collection date")+
  coord_cartesian(xlim=c(as.Date("2021-01-01"),as.Date(date.to, origin="1970-01-01")), ylim=c(0.001, 0.995), expand=0)
plot_southafrica_mfit_logit
...
qplot(data=data_prov_wide, x=date, y=daily_cases, geom="col", colour=I("blue")) + facet_wrap(~ province, scale="free_y")  + 
  ylab("new cases per day") + xlab("reporting date") + ggtitle("New confirmed Covid cases per day\nin South Africa") + xaxis + coord_cartesian(xlim=c(as.Date("2020-01-01"),as.Date("2022-07-25"))) # + scale_y_log10()
...
cases_tot$cases_new[cases_tot$cases_new<0] = 0
library(mgcv)
k=50
...
infections_IHME = read.csv(".//data//daily-new-estimated-covid-19-infections-ihme-model_10 June 2022.csv")
...
sum(fitted(fit_infections)) # 80649247, SA population is 60,652,262 as of Thursday, April 21, 2022, based on Worldometer elaboration of the latest United Nations data
sum(cases_tot$infections_IHME, na.rm=T) # 81417906
...
Re_from_r <- function(r, gamma_mean=4.7, gamma_sd=2.9) { # Nishiura et al. 2020, or use values from Ferretti et al. 2020 (gamma_mean=5.5, gamma_sd=1.8)
...
avg_r_cases$Re = Re_from_r(avg_r_cases$r)
avg_r_cases$Re_LOWER = Re_from_r(avg_r_cases$r_LOWER)
avg_r_cases$Re_UPPER = Re_from_r(avg_r_cases$r_UPPER)
...
avg_r_cases_gauteng$Re = Re_from_r(avg_r_cases_gauteng$r)
avg_r_cases_gauteng$Re_LOWER = Re_from_r(avg_r_cases_gauteng$r_LOWER)
avg_r_cases_gauteng$Re_UPPER = Re_from_r(avg_r_cases_gauteng$r_UPPER)
...
above_avg_r_variants$Re = Re_from_r(above_avg_r_variants$r)
above_avg_r_variants$Re_LOWER = Re_from_r(above_avg_r_variants$r_LOWER)
above_avg_r_variants$Re_UPPER = Re_from_r(above_avg_r_variants$r_UPPER)
