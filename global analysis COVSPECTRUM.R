...
target_dir = "./data/COVSPECTRUM"
suppressWarnings(dir.create(target_dir))
plotdir = "./plots/COVSPECTRUM"
...
qplot(data=meffects_sel1, 
  ...
  ggtitle("GROWTH RATE ADVANTAGE OF SARS-CoV2 VARIANTS",
          subtitle=paste0("based on multinomial fit ", model, "\nGISAID data, using data from countries with >=", minseqs, " XBB.1.5* sequences") ) +
  labs(tag = tag) +
...
qplot(data=meffects_sel2, 
  ...
  ggtitle("GROWTH RATE ADVANTAGE OF SARS-CoV2 VARIANTS",
          subtitle=paste0("based on multinomial fit ", model, "\nGISAID data, using data from countries with >=", minseqs, " XBB.1.5* sequences\n", 
                          "Estimates shown for regions with >=",minvariantseqs," sequences of each variant") ) +
  labs(tag = tag) +
  theme(plot.tag.position = "bottomright",
        plot.tag = element_text(vjust = 1, hjust = 1, size=8)) +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
...
qplot(data=meffects_sel2[meffects_sel2$region==reg,], 
  ...
  scale_fill_manual(values=cols) +
  theme(legend.position="none") + xlab("") +
  ggtitle(paste0("GROWTH RATE ADVANTAGE OF SARS-CoV2 VARIANTS IN ", toupper(reg)),
          subtitle=paste0("based on multinomial fit ", model, "\nGISAID data, using data from countries with >=", minseqs, " XBB.1.5* sequences\n") 
          ) +
  labs(tag = tag) +
  theme(plot.tag.position = "bottomright",
...
pl = qplot(data=fit_preds[fit_preds$variant!="Other",], 
  ...
  ggtitle("SARS-CoV2 LINEAGE FREQUENCIES",
          subtitle=paste0("GISAID data up to ",today, ", multinomial fit ", model, ",\nall countries with >=", minseqs, " XBB.1.5* sequences included")) +
  scale_x_date(date_breaks = "3 months", date_labels =  "%b %Y", expand=FALSE) +   
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust=1)) +  
  scale_y_continuous( trans="logit", breaks=c(10^seq(-5,0),0.5,0.9,0.99,0.999),
                      labels = c("0.001","0.01","0.1","1","10","100","50","90","99","99.9")) +
  scale_fill_manual("variant", values=tail(as.vector(lineage_cols),-1)) +
...
ggsave(file=file.path(plotdir, "predicted lineage freqs_selected states countries_logit scale.png"), 
       width=3*ncls, 
       height=1*ncls)
...
ggsave(file=file.path(plotdir, "predicted lineage freqs_selected states countries_last6months_logit scale.png"), 
       width=3*ncls, 
       height=1*ncls)
