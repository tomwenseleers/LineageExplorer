...
main_lineages
# "B.1"      "B.1.1"    "B.1.1.7"  "B.1.160"  "B.1.177+" "B.1.91" 
...
table(GISAID_sel$LINEAGE1)
#   B.1    B.1.1  B.1.1.7  B.1.160 B.1.177+  B.1.351 B.1.617+   B.1.91    other      P.1 
#   475      823     4206      297     1319       97      126      288     1061       83 
...
# "B.1.1.7"  "B.1"      "B.1.1"    "B.1.160"  "B.1.177+" "B.1.351"  "B.1.617+" "B.1.91"   "other"    "P.1"      
levels_LINEAGE1 = c("B.1.1.7","B.1","B.1.1","B.1.91","B.1.160","B.1.177+",
...
levels_LINEAGE2 = c("B.1.1.7","B.1","B.1.1","B.1.91","B.1.160","B.1.177+",
...
table(GISAID_sel$LINEAGE2)
# B.1.1.7       B.1     B.1.1    B.1.91   B.1.160  B.1.177+   B.1.351       P.1 B.1.617.1 B.1.617.2     other 
# 4206       475       823       288       297      1319        97        83         5       113      1061 

range(GISAID_sel$date) # "2020-03-01" "2021-06-02"
...
muller_portugal_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN PORTUGAL\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_portugal_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_muller plots_raw data.png"), width=8, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_muller plots_raw data.pdf"), width=8, height=6)
...
library(splines)
set.seed(1)
fit1_portugal_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_portugal_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_portugal_multi, fit2_portugal_multi) 
#             df      BIC
# fit1_portugal_multi 20 19432.42
# fit2_portugal_multi 30 18108.59

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrportugal = emtrends(fit2_portugal_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_portugal = data.frame(confint(emtrportugal, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrportugal$contrasts)$p.value)
delta_r_portugal
#                         contrast      estimate         SE df   asymp.LCL  asymp.UCL   p.value
# 1         B.1 - B.1.1.7  0.018723326 0.002997714 NA  0.0128479139  0.024598738 6.690668e-06
# 2       B.1.1 - B.1.1.7  0.004348189 0.003157964 NA -0.0018413074  0.010537685 6.679802e-01
# 3      B.1.91 - B.1.1.7 -2.094062251 0.610407317 NA -3.2904386078 -0.897685895 1.451380e-02
# 4     B.1.160 - B.1.1.7 -0.057806179 0.006615357 NA -0.0707720414 -0.044840317 9.355559e-09
# 5  (B.1.177+) - B.1.1.7 -0.049085315 0.004001470 NA -0.0569280534 -0.041242578 3.216760e-12
# 6     B.1.351 - B.1.1.7 -0.034698084 0.009832885 NA -0.0539701839 -0.015425984 1.132932e-02
# 7         P.1 - B.1.1.7 -0.009705042 0.010763554 NA -0.0308012207  0.011391136 9.036768e-01
# 8   B.1.617.1 - B.1.1.7  0.118817059 0.042360217 NA  0.0357925593  0.201841560 6.408985e-02
# 9   B.1.617.2 - B.1.1.7  0.187699759 0.013741819 NA  0.1607662896  0.214633229 2.500222e-13
# 10      other - B.1.1.7  0.005188055 0.002383946 NA  0.0005156061  0.009860504 2.263119e-01


# fitted prop of different LINEAGES in the portugal today
# 99% [97%-100%] now estimated to be B.1.617.2 across all regions
multinom_preds_today_avg = data.frame(emmeans(fit2_portugal_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
#    LINEAGE2         prob           SE df    asymp.LCL    asymp.UCL
# 1    B.1.1.7  6.194358e-03 3.165578e-03 NA  -1.006092e-05  1.239878e-02
# 2        B.1  2.558412e-04 1.471714e-04 NA  -3.260947e-05  5.442919e-04
# 3      B.1.1  5.101797e-05 3.128914e-05 NA  -1.030762e-05  1.123436e-04
# 4     B.1.91 2.826445e-216 0.000000e+00 NA  2.826445e-216 2.826445e-216
# 5    B.1.160  3.620371e-07 3.141579e-07 NA  -2.537010e-07  9.777752e-07
# 6   B.1.177+  3.144331e-06 2.002986e-06 NA  -7.814493e-07  7.070112e-06
# 7    B.1.351  1.879964e-05 1.517144e-05 NA  -1.093585e-05  4.853512e-05
# 8        P.1  1.154705e-04 8.939532e-05 NA  -5.974108e-05  2.906821e-04
# 9  B.1.617.1  3.730184e-03 6.513076e-03 NA  -9.035210e-03  1.649558e-02
# 10 B.1.617.2  9.890424e-01 8.044352e-03 NA   9.732758e-01  1.004809e+00
# 11     other  5.883806e-04 3.158215e-04 NA  -3.061826e-05  1.207379e-03

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9938056 0.9640953 1.0235159 
...
fit_portugal_multi_preds = data.frame(predgrid, as.data.frame(predict(fit2_portugal_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_portugal_multi_preds = gather(fit_portugal_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_portugal_multi_preds$collection_date = as.Date(fit_portugal_multi_preds$DATE_NUM, origin="1970-01-01")
fit_portugal_multi_preds$LINEAGE2 = factor(fit_portugal_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 

muller_portugal_mfit = ggplot(data=fit_portugal_multi_preds, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN PORTUGAL\n(GISAID data, multinomial fit)")
muller_portugal_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_muller plots_multinom fit.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_muller plots_multinom fit.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_portugal_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_portugal_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_muller plots multipanel_multinom fit.png"), width=10, height=10)
ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_muller plots multipanel_multinom fit.pdf"), width=10, height=10)
...
fit_portugal_multi_preds_bystate_withCI = data.frame(emmeans(fit3_portugal_multi,
                                                        ~ LINEAGE2,
                                                        by=c("DATE_NUM","STATE"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                        mode="prob", df=NA))
fit_portugal_multi_preds_bystate_withCI$collection_date = as.Date(fit_portugal_multi_preds_bystate_withCI$DATE_NUM, origin="1970-01-01")
fit_portugal_multi_preds_bystate_withCI$LINEAGE2 = factor(fit_portugal_multi_preds_bystate_withCI$LINEAGE2, levels=levels_LINEAGE2)
fit_portugal_multi_preds2 = fit_portugal_multi_preds_bystate_withCI

fit_portugal_multi_preds_bystate_withCI[fit_portugal_multi_preds_bystate_withCI$collection_date==as.Date("2021-06-21")&fit_portugal_multi_preds_bystate_withCI$LINEAGE2=="B.1.617.2",]
...
# fit_portugal_multi_preds2 = fit_portugal_multi_preds_bystate # without CIs
# fit_portugal_multi_preds2$asymp.LCL = NA
# fit_portugal_multi_preds2$asymp.UCL = NA
...
ymin = 0.001
ymax = 0.999
fit_portugal_multi_preds2$asymp.LCL[fit_portugal_multi_preds2$asymp.LCL<ymin] = ymin
fit_portugal_multi_preds2$asymp.UCL[fit_portugal_multi_preds2$asymp.UCL<ymin] = ymin
fit_portugal_multi_preds2$asymp.UCL[fit_portugal_multi_preds2$asymp.UCL>ymax] = ymax
fit_portugal_multi_preds2$prob[fit_portugal_multi_preds2$prob<ymin] = ymin

plot_portugal_mfit_logit = qplot(data=fit_portugal_multi_preds2, x=collection_date, y=prob, geom="blank") +
  facet_wrap(~ STATE) +
  ...
  ylab("Share (%)") +
  theme_hc() + xlab("") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN THE portugal\n(GISAID data, multinomial fit)") +
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01")),
                     labels=substring(months(as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01"))),1,1),
                     limits=as.Date(c("2020-11-01",NA)), expand=c(0,0)) +
...
plot_portugal_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_multinom fit by state_logit scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_multinom fit by state_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_portugal_mfit = qplot(data=fit_portugal_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  facet_wrap(~ STATE) +
...
ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_multinom fit by state_response scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_multinom fit by state_response scale.pdf"), width=10, height=6)




# overall multinomial model predictions with confidence intervals
fit_portugal_multi_preds_withCI = data.frame(emmeans(fit3_portugal_multi,
                                                        ~ LINEAGE2,
                                                        by=c("DATE_NUM"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                        mode="prob", df=NA))
fit_portugal_multi_preds_withCI$collection_date = as.Date(fit_portugal_multi_preds_withCI$DATE_NUM, origin="1970-01-01")
fit_portugal_multi_preds_withCI$LINEAGE2 = factor(fit_portugal_multi_preds_withCI$LINEAGE2, levels=levels_LINEAGE2)
fit_portugal_multi_preds3 = fit_portugal_multi_preds_withCI

fit_portugal_multi_preds_withCI[fit_portugal_multi_preds_withCI$collection_date==as.Date("2021-06-21")&fit_portugal_multi_preds_withCI$LINEAGE2=="B.1.617.2",]
#        LINEAGE2 DATE_NUM         STATE       prob          SE df  asymp.LCL  asymp.UCL collection_date
# 615 B.1.617.2    18799 0.4215232 0.01522866 NA 0.3916756 0.4513708      2021-06-21

# fit_portugal_multi_preds2 = fit_portugal_multi_preds # without CIs
# fit_portugal_multi_preds2$asymp.LCL = NA
# fit_portugal_multi_preds2$asymp.UCL = NA
...
ymin = 0.001
ymax = 0.999
fit_portugal_multi_preds3$asymp.LCL[fit_portugal_multi_preds3$asymp.LCL<ymin] = ymin
fit_portugal_multi_preds3$asymp.UCL[fit_portugal_multi_preds3$asymp.UCL<ymin] = ymin
fit_portugal_multi_preds3$asymp.UCL[fit_portugal_multi_preds3$asymp.UCL>ymax] = ymax
fit_portugal_multi_preds3$prob[fit_portugal_multi_preds3$prob<ymin] = ymin

plot_portugal_avg_mfit_logit = qplot(data=fit_portugal_multi_preds3, x=collection_date, y=prob, geom="blank") +
  # facet_wrap(~ STATE) +
  geom_ribbon(aes(y=prob, ymin=asymp.LCL, ymax=asymp.UCL, colour=NULL,
                  fill=LINEAGE2
  ), alpha=I(0.3)) +
  ...
  xlab("Collection date")+
  coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")), ylim=c(0.001, 0.9901), expand=c(0,0))
plot_portugal_avg_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_multinom fit_logit scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_multinom fit_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_portugal_avg_mfit = qplot(data=fit_portugal_multi_preds3, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ STATE) +
  geom_ribbon(aes(y=100*prob, ymin=100*asymp.LCL, ymax=100*asymp.UCL, colour=NULL,
                  fill=LINEAGE2
  ), alpha=I(0.3)) +
  ...
  theme(legend.position = "right") +
  xlab("Collection date")
plot_portugal_avg_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_multinom fit_response scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\portugal_multinom fit_response scale.pdf"), width=10, height=6)
