# ANALYSIS OF GROWTH ADVANTAGE OF DIFFERENT VOCs IN FRANCE (GISAID GENOMIC EPIDEMIOLOGY METADATA)
# T. Wenseleers
...
# [9] "Italy"          "Japan"          "France"       "France"         "Singapore"      "France"          "Switzerland"    "United Kingdom"
# [17] "USA"    
...
# ANALYSIS OF VOCs IN FRANCE ####

sel_countries = "France"
...
nrow(GISAID_sel) # 42712

GISAID_sel = GISAID_sel[-which(grepl("HDF-[0-9]",GISAID_sel$strain)),] # these records were wrong, see https://twitter.com/flodebarre/status/1410154017854996480
nrow(GISAID_sel) # 39276
...
table(GISAID_sel$LINEAGE1)
# B.1.1.519   B.1.1.7   B.1.160   B.1.351   B.1.525  B.1.617+     other       P.1 
# 42     29526      2578      2320       626       323      6842       455 
...
# "B.1.1.7"   "B.1.1.519" "B.1.160"   "B.1.351"   "B.1.525"   "B.1.617+"  "other"     "P.1" 
levels_LINEAGE1 = c("B.1.1.7","B.1.1.519","B.1.160","B.1.525",
...
# "B.1.1.7"   "B.1.1.519" "B.1.160"   "B.1.351"   "B.1.525"   "B.1.617.1" "B.1.617.2" "other"     "P.1"  
levels_LINEAGE2 = c("B.1.1.7","B.1.1.519","B.1.160","B.1.525",
...
range(GISAID_sel$date) # "2020-01-01" "2021-06-24"
...
qplot(data=data_agbyday2, x=date, y=total, colour=total>300, fill=total>300, geom="col") # rough identification of "Flash" baseline surveillance campaigns
GISAID_sel$total_sequenced_on_that_day = data_agbyday2$total[match(GISAID_sel$date, data_agbyday2$date)]
GISAID_sel = GISAID_sel[GISAID_sel$total_sequenced_on_that_day>300,] # dates on which baseline surveillance was performed
nrow(GISAID_sel) # 20617
...
muller_france_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN FRANCE\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_france_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\france_muller plots_raw data.png"), width=8, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\france_muller plots_raw data.pdf"), width=8, height=6)
...
library(splines)
set.seed(1)
fit1_france_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_france_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_france_multi, fit2_france_multi) 
# df      BIC
# fit1_france_multi 16 34005.33
# fit2_france_multi 24 33873.85

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrfrance = emtrends(fit2_france_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_france = data.frame(confint(emtrfrance, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrfrance$contrasts)$p.value)
delta_r_france
# contrast     estimate           SE df    asymp.LCL    asymp.UCL      p.value
# 1 B.1.1.519 - B.1.1.7 -5.265624e-02 0.035863437 NA -0.122947286  0.017634803 5.638846e-01
# 2   B.1.160 - B.1.1.7 -8.480604e-02 0.010623707 NA -0.105628121 -0.063983954 2.516788e-07
# 3   B.1.525 - B.1.1.7 -1.700857e-02 0.005440227 NA -0.027671215 -0.006345916 2.914746e-02
# 4   B.1.351 - B.1.1.7  1.601807e-03 0.002372556 NA -0.003048318  0.006251932 9.470751e-01
# 5       P.1 - B.1.1.7  3.297905e-03 0.006860925 NA -0.010149260  0.016745070 9.824024e-01
# 6 B.1.617.1 - B.1.1.7 -1.725211e-01 0.132719679 NA -0.432646861  0.087604720 6.664007e-01
# 7 B.1.617.2 - B.1.1.7  7.770430e-02 0.010937886 NA  0.056266435  0.099142159 1.839675e-06
# 8     other - B.1.1.7 -7.972896e-05 0.001950966 NA -0.003903551  0.003744093 9.999967e-01


# fitted prop of different LINEAGES in the france today
multinom_preds_today_avg = data.frame(emmeans(fit2_france_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1   B.1.1.7 6.761222e-01 5.063040e-02 NA  5.768884e-01 7.753559e-01
# 2 B.1.1.519 1.033172e-05 2.763503e-05 NA -4.383195e-05 6.449539e-05
# 3   B.1.160 1.329085e-05 1.119316e-05 NA -8.647341e-06 3.522904e-05
# 4   B.1.525 7.807329e-03 2.397844e-03 NA  3.107640e-03 1.250702e-02
# 5   B.1.351 5.144572e-02 7.866755e-03 NA  3.602716e-02 6.686427e-02
# 6       P.1 8.478333e-03 3.268991e-03 NA  2.071228e-03 1.488544e-02
# 7 B.1.617.1 1.807308e-07 1.164805e-06 NA -2.102244e-06 2.463706e-06
# 8 B.1.617.2 2.076994e-01 5.829032e-02 NA  9.345242e-02 3.219463e-01
# 9     other 4.842327e-02 6.928104e-03 NA  3.484444e-02 6.200210e-02

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.3238778 0.1694483 0.4783073 
...
fit_france_multi_preds = data.frame(predgrid, as.data.frame(predict(fit2_france_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_france_multi_preds = gather(fit_france_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_france_multi_preds$collection_date = as.Date(fit_france_multi_preds$DATE_NUM, origin="1970-01-01")
fit_france_multi_preds$LINEAGE2 = factor(fit_france_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 

muller_france_mfit = ggplot(data=fit_france_multi_preds, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN FRANCE\n(GISAID data, multinomial fit)")
muller_france_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\france_muller plots_multinom fit.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\france_muller plots_multinom fit.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_france_raw2 + coord_cartesian(xlim=c(as.Date("2021-01-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_france_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\france_muller plots multipanel_multinom fit.png"), width=10, height=10)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\france_muller plots multipanel_multinom fit.pdf"), width=10, height=10)
...
fit_france_multi_preds_withCI = data.frame(emmeans(fit2_france_multi,
                                                        ~ LINEAGE2,
                                                        by=c("DATE_NUM"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=1)),  # by=XX to speed up things a bit
                                                        mode="prob", df=NA))
fit_france_multi_preds_withCI$collection_date = as.Date(fit_france_multi_preds_withCI$DATE_NUM, origin="1970-01-01")
fit_france_multi_preds_withCI$LINEAGE2 = factor(fit_france_multi_preds_withCI$LINEAGE2, levels=levels_LINEAGE2)
fit_france_multi_preds2 = fit_france_multi_preds_withCI
...
ymin = 0.001
ymax = 0.999
fit_france_multi_preds2$asymp.LCL[fit_france_multi_preds2$asymp.LCL<ymin] = ymin
fit_france_multi_preds2$asymp.UCL[fit_france_multi_preds2$asymp.UCL<ymin] = ymin
fit_france_multi_preds2$asymp.UCL[fit_france_multi_preds2$asymp.UCL>ymax] = ymax
fit_france_multi_preds2$prob[fit_france_multi_preds2$prob<ymin] = ymin

plot_france_mfit_logit = qplot(data=fit_france_multi_preds2, x=collection_date, y=prob, geom="blank") +
  # facet_wrap(~ STATE) +
...
plot_france_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\france_multinom fit_logit scale.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\france_multinom fit_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_france_mfit = qplot(data=fit_france_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ STATE) +
...
ggsave(file=paste0(".\\plots\\",plotdir,"\\france_multinom fit_response scale.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\france_multinom fit_response scale.pdf"), width=10, height=6)
...
cases_tot = as.data.frame(get_national_data(countries = "France"))
...
fit_france_multi_preds_withCI$totcases = cases_tot$cases_new[match(round(fit_france_multi_preds_withCI$DATE_NUM),cases_tot$DATE_NUM)]
fit_france_multi_preds_withCI$cases = fit_france_multi_preds_withCI$totcases * fit_france_multi_preds_withCI$prob
fit_france_multi_preds_withCI$cases[fit_france_multi_preds_withCI$cases<=0.001] = NA
...
fit_france_multi_preds_withCI$smoothed_totcases = cases_emmeans$rate[match(fit_france_multi_preds_withCI$DATE_NUM,cases_emmeans$DATE_NUM)]
fit_france_multi_preds_withCI$smoothed_cases = fit_france_multi_preds_withCI$smoothed_totcases * fit_france_multi_preds_withCI$prob
fit_france_multi_preds_withCI$smoothed_cases[fit_france_multi_preds_withCI$smoothed_cases<=0.001] = NA

ggplot(data=fit_france_multi_preds_withCI[fit_france_multi_preds_withCI$collection_date>=as.Date("2021-02-18"),], 
...
ggplot(data=fit_france_multi_preds_withCI[fit_france_multi_preds_withCI$collection_date>=as.Date("2021-02-18")&
                                              fit_france_multi_preds_withCI$collection_date<=max(cases_tot$date),], 
...
qplot(data=avg_r_cases, x=DATE-7, y=Re, ymin=Re_LOWER, ymax=Re_UPPER, geom="ribbon", alpha=I(0.5), fill=I("steelblue")) +
  ...
  ggtitle("Re IN FRANCE AT MOMENT OF INFECTION BASED ON NEW CASES") +
  # labs(tag = tag) +
...
above_avg_r_variants0 = do.call(rbind, lapply(seq(date.from,
                                                  date.to), 
                                              function (d) { 
                                                wt = as.data.frame(emmeans(fit2_france_multi, ~ LINEAGE2 , at=list(DATE_NUM=d), type="response"))$prob   # important: these should sum to 1
                                                ...
                                                EMT = emtrends(fit2_france_multi,  ~ LINEAGE2 , by=c("DATE_NUM"),
                                                               var="DATE_NUM", mode="latent",
                                                               at=list(DATE_NUM=d))
...
above_avg_r_variants$prob = fit_france_multi_preds_withCI$prob[match(interaction(above_avg_r_variants$DATE_NUM,
                                                                      above_avg_r_variants$variant),
                                                          interaction(fit_france_multi_preds_withCI$DATE_NUM,
                                                                      fit_france_multi_preds_withCI$LINEAGE2))]
