# ANALYSIS OF GROWTH ADVANTAGE OF DIFFERENT VOCs IN GREECE (GISAID RECORDS)
# T. Wenseleers
...
plotdir = "Greece_GISAID"
suppressWarnings(dir.create(paste0(".//plots//",plotdir)))

# import GISAID records for Greece
GISAID = read_tsv(file(".//data//GISAID//Greece//gisaid_hcov-19_2021_07_15_16.tsv"), col_types = cols(.default = "c")) 
...
range(GISAID$date) # "2020-02-29" "2021-06-07"
...
# sel_countries = "Greece"
# GISAID[GISAID$country %in% sel_countries,]

GISAID_sel = GISAID

sum(GISAID_sel$LINEAGE=="B.1.617.2") # 10
...
GISAID_sel$LINEAGE[(GISAID_sel$LINEAGE %in% remove2)] = "other" # minority VOCs
table(GISAID_sel$LINEAGE)
# B.1.1.318   B.1.1.7  B.1.177+   B.1.351   B.1.525 B.1.617.1 B.1.617.2     other 
# 751      5455       534        21         2         1        10      1131 
...
# "B.1.1.7"   "B.1.1.318" "B.1.177+"  "B.1.351"   "B.1.525"   "B.1.617.1" "B.1.617.2" "other"
levels_LINEAGE = c("B.1.1.7","B.1.1.525","B.1.177+","B.1.1.318",
...
muller_greece_raw2 = ggplot(data=data_agbyweek, aes(x=collection_date, y=count, group=LINEAGE)) + 
...
library(splines)
set.seed(1)
fit1_greece_multi = nnet::multinom(LINEAGE ~ scale(DATE_NUM), weights=count, data=data_agbyweek, maxit=1000)
fit2_greece_multi = nnet::multinom(LINEAGE ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek, maxit=1000)
fit3_greece_multi = nnet::multinom(LINEAGE ~ ns(DATE_NUM, df=3), weights=count, data=data_agbyweek, maxit=1000)
BIC(fit1_greece_multi, fit2_greece_multi, fit3_greece_multi) 
# df      BIC
# fit1_greece_multi 16 9227.951
# fit2_greece_multi 24 9157.387
# fit3_greece_multi 32 9201.032

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrgreece = emtrends(fit2_greece_multi, trt.vs.ctrl ~ LINEAGE,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_greece = data.frame(confint(emtrgreece, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrgreece$contrasts)$p.value)
delta_r_greece
# contrast     estimate          SE df   asymp.LCL    asymp.UCL      p.value
# 1  B.1.1.525 - B.1.1.7 -0.014032381 0.191371225 NA -0.38911309  0.361048327 9.999744e-01
# 2 (B.1.177+) - B.1.1.7 -0.021918625 0.004317690 NA -0.03038114 -0.013456109 2.508049e-04
# 3  B.1.1.318 - B.1.1.7  0.036212123 0.004340293 NA  0.02770531  0.044718940 1.145136e-07
# 4    B.1.351 - B.1.1.7 -0.080366522 0.029727123 NA -0.13863061 -0.022102431 7.329397e-02
# 5        P.1 - B.1.1.7 -0.014032381 0.191371225 NA -0.38911309  0.361048327 9.999744e-01
# 6  B.1.617.1 - B.1.1.7 -0.003936702 0.036433966 NA -0.07534596  0.067472559 9.998997e-01
# 7  B.1.617.2 - B.1.1.7  0.009103214 0.014375910 NA -0.01907305  0.037279481 9.567919e-01
# 8      other - B.1.1.7 -0.013997116 0.004124497 NA -0.02208098 -0.005913251 1.574517e-02


# fitted prop of different LINEAGES in the greece today
# 96.5% [95.7%-97.3%] now estimated to be B.1.617.2 across all regions
multinom_preds_today_avg = data.frame(emmeans(fit2_greece_multi, ~ LINEAGE|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE         prob           SE df     asymp.LCL    asymp.UCL
# 1   B.1.1.7 2.392581e-01 5.116694e-02 NA  1.389727e-01 3.395434e-01
# 2 B.1.1.525 1.440416e-10 4.080622e-08 NA -7.983468e-08 8.012276e-08
# 3  B.1.177+ 3.500130e-04 1.799493e-04 NA -2.681180e-06 7.027073e-04
# 4 B.1.1.318 7.581457e-01 5.170671e-02 NA  6.568024e-01 8.594890e-01
# 5   B.1.351 8.056267e-08 2.703725e-07 NA -4.493576e-07 6.104830e-07
# 6       P.1 1.440416e-10 4.080622e-08 NA -7.983468e-08 8.012276e-08
# 7 B.1.617.1 3.414401e-05 1.250173e-04 NA -2.108854e-04 2.791734e-04
# 8 B.1.617.2 1.413644e-03 1.705313e-03 NA -1.928708e-03 4.755996e-03
# 9     other 7.983452e-04 3.878402e-04 NA  3.819236e-05 1.558498e-03

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.7607419 0.6546977 0.8667861
...
fit_greece_multi_preds = data.frame(predgrid, as.data.frame(predict(fit2_greece_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_greece_multi_preds = gather(fit_greece_multi_preds, LINEAGE, prob, all_of(levels_LINEAGE), factor_key=TRUE)
fit_greece_multi_preds$collection_date = as.Date(fit_greece_multi_preds$DATE_NUM, origin="1970-01-01")
fit_greece_multi_preds$LINEAGE = factor(fit_greece_multi_preds$LINEAGE, levels=levels_LINEAGE) 
...
fit_greece_multi_preds_withCI = data.frame(emmeans(fit2_greece_multi,
                                                        ~ LINEAGE,
                                                        by=c("DATE_NUM"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=1)),  # by=XX to speed up things a bit
                                                        mode="prob", df=NA))
fit_greece_multi_preds_withCI$collection_date = as.Date(fit_greece_multi_preds_withCI$DATE_NUM, origin="1970-01-01")
fit_greece_multi_preds_withCI$LINEAGE = factor(fit_greece_multi_preds_withCI$LINEAGE, levels=levels_LINEAGE)
...
plot_greece_mfit_logit = qplot(data=fit_greece_multi_preds2, x=collection_date, y=prob, geom="blank") +
  # facet_wrap(~ STATE) +
...
# on response scale:
plot_greece_mfit = qplot(data=fit_greece_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ STATE) +
...
ggplot(data=fit_greece_multi_preds_withCI[fit_greece_multi_preds_withCI$collection_date>=as.Date("2021-01-01"),], 
  ...
  scale_fill_manual("variant", values=lineage_cols2[-c(2,5)]) +
  scale_colour_manual("variant", values=lineage_cols2[-c(2,5)]) +
...
ggplot(data=fit_greece_multi_preds_withCI[fit_greece_multi_preds_withCI$collection_date>=as.Date("2021-01-01")&
                                              fit_greece_multi_preds_withCI$collection_date<=max(cases_tot$date),], 
...
above_avg_r_variants0 = do.call(rbind, lapply(seq(date.from,
                                                  date.to), 
                                              function (d) { 
                                                wt = as.data.frame(emmeans(fit2_greece_multi, ~ LINEAGE , at=list(DATE_NUM=d), type="response"))$prob   # important: these should sum to 1
                                                ...
                                                EMT = emtrends(fit2_greece_multi,  ~ LINEAGE , by=c("DATE_NUM"),
                                                               var="DATE_NUM", mode="latent",
                                                               at=list(DATE_NUM=d))
...
above_avg_r_variants$prob = fit_greece_multi_preds_withCI$prob[match(interaction(above_avg_r_variants$DATE_NUM,
                                                          ...
                                                          interaction(fit_greece_multi_preds_withCI$DATE_NUM,
                                                                      fit_greece_multi_preds_withCI$LINEAGE))]
