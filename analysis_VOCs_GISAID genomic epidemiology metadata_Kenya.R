...
sel_countries = c("Kenya") # "Democratic Republic of the Congo","Uganda","Malawi","   "Angola","Senegal" "Botswana"
...
# muller plot, overall
muller_kenya_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN KENYA\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_kenya_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_muller plots_raw data.png"), width=8, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_muller plots_raw data.pdf"), width=8, height=6)
...
library(splines)
set.seed(1)
fit1_kenya_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_kenya_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_kenya_multi, fit2_kenya_multi) 
#             df      BIC
# fit1_kenya_multi 16 2307.474
# fit2_kenya_multi 24 2333.767

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrkenya = emtrends(fit1_kenya_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_kenya = data.frame(confint(emtrkenya, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrkenya$contrasts)$p.value)
delta_r_kenya
# contrast     estimate           SE df    asymp.LCL     asymp.UCL      p.value
# 1       B.1 - B.1.1.7 -0.06935597 0.004990749 NA -0.07913765 -0.05957428 1.792017e-09
# 2     B.1.1 - B.1.1.7 -0.05049262 0.008016729 NA -0.06620512 -0.03478012 7.545243e-05
# 3   B.1.351 - B.1.1.7 -0.01682744 0.003475917 NA -0.02364011 -0.01001477 1.234578e-03
# 4   B.1.530 - B.1.1.7 -0.07109789 0.005565588 NA -0.08200625 -0.06018954 6.171710e-09
# 5   B.1.549 - B.1.1.7 -0.09390657 0.008558906 NA -0.11068172 -0.07713142 5.502306e-08
# 6 B.1.617.1 - B.1.1.7  0.02322082 0.019423846 NA -0.01484922  0.06129085 7.293128e-01
# 7 B.1.617.2 - B.1.1.7  0.05843954 0.010170013 NA  0.03850668  0.07837240 2.113906e-04
# 8     other - B.1.1.7 -0.05942478 0.005032420 NA -0.06928814 -0.04956142 1.925488e-08


# fitted prop of different LINEAGES in the kenya today
multinom_preds_today_avg = data.frame(emmeans(fit1_kenya_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1   B.1.1.7 2.627481e-01 9.169053e-02 NA  8.303797e-02 4.424582e-01
# 2       B.1 4.324798e-05 2.898170e-05 NA -1.355511e-05 1.000511e-04
# 3     B.1.1 4.724142e-05 5.257363e-05 NA -5.580100e-05 1.502838e-04
# 4   B.1.351 3.110856e-02 1.333598e-02 NA  4.970524e-03 5.724660e-02
# 5   B.1.530 1.199739e-05 9.757395e-06 NA -7.126752e-06 3.112153e-05
# 6   B.1.549 7.812601e-08 1.242843e-07 NA -1.654668e-07 3.217188e-07
# 7 B.1.617.1 1.595434e-02 1.960884e-02 NA -2.247827e-02 5.438695e-02
# 8 B.1.617.2 6.899899e-01 1.069647e-01 NA  4.803430e-01 8.996368e-01
# 9     other 9.649487e-05 6.518666e-05 NA -3.126862e-05 2.242584e-04

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.7372519 0.4627274 1.0117764 
...
fit_kenya_multi_preds_withCI = data.frame(emmeans(fit1_kenya_multi,
                                                           ~ LINEAGE2,
                                                           by=c("DATE_NUM"),
                                                           at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                           mode="prob", df=NA))
fit_kenya_multi_preds_withCI$collection_date = as.Date(fit_kenya_multi_preds_withCI$DATE_NUM, origin="1970-01-01")
fit_kenya_multi_preds_withCI$LINEAGE2 = factor(fit_kenya_multi_preds_withCI$LINEAGE2, levels=levels_LINEAGE2)

muller_kenya_mfit = ggplot(data=fit_kenya_multi_preds_withCI, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN KENYA\n(GISAID data, multinomial fit)")
muller_kenya_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_muller plot_multinom fit avg.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_muller plot_multinom fit avg.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_kenya_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_kenya_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_muller plot multipanel_multinom fit avg.png"), width=10, height=10)
ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_muller plot multipanel_multinom fit avg.pdf"), width=10, height=10)
...
fit_kenya_multi_preds_withCI[fit_kenya_multi_preds_withCI$collection_date==(as.Date("2021-06-22")-2)&fit_kenya_multi_preds_withCI$LINEAGE2=="B.1.617.2",]
...
# fit_kenya_multi_preds2 = fit_kenya_multi_preds_bystate # without CIs
# fit_kenya_multi_preds2$asymp.LCL = NA
# fit_kenya_multi_preds2$asymp.UCL = NA


# on logit scale:

fit_kenya_multi_preds2 = fit_kenya_multi_preds_withCI
ymin = 0.001
ymax = 0.999
fit_kenya_multi_preds2$asymp.LCL[fit_kenya_multi_preds2$asymp.LCL<ymin] = ymin
fit_kenya_multi_preds2$asymp.UCL[fit_kenya_multi_preds2$asymp.UCL<ymin] = ymin
fit_kenya_multi_preds2$asymp.UCL[fit_kenya_multi_preds2$asymp.UCL>ymax] = ymax
fit_kenya_multi_preds2$prob[fit_kenya_multi_preds2$prob<ymin] = ymin

plot_kenya_mfit_logit = qplot(data=fit_kenya_multi_preds2, x=collection_date, y=prob, geom="blank") +
  # facet_wrap(~ country) +
...
plot_kenya_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_multinom fit avg_logit scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_multinom fit avg_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_kenya_mfit = qplot(data=fit_kenya_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ country) +
...
ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_multinom fit avg_response scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\kenya_multinom fit avg_response scale.pdf"), width=10, height=6)
