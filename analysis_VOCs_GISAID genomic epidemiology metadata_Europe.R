# ANALYSIS OF GROWTH ADVANTAGE OF DIFFERENT VOCs IN EUROPE (GISAID GENOMIC EPIDEMIOLOGY METADATA)
# T. Wenseleers
# last update 14 SEPT 2021
...
plotdir = "Europe_GISAID_genomic_epidemiology"
suppressWarnings(dir.create(paste0(".//plots//",plotdir)))

# import GISAID genomic epidemiology metadata
GISAID = read_tsv(gzfile(".//data//GISAID//metadata.tsv"), col_types = cols(.default = "c")) 
...
range(GISAID$date) # "2020-01-01" "2021-09-07"
nrow(GISAID) #  3216793
...
GISAID$pango_lineage[grepl("AY.33",GISAID$pango_lineage)] = "ay.33"
...
# ANALYSIS VOCs IN THE Europe ####
sel_countries = "Europe"
...
GISAID_sel = GISAID_sel[!grepl("Europe",GISAID_sel$state),] # remove data with unspecified state
...
muller_sel_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN THE Europe\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_sel_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots_raw data.png"), width=8, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots_raw data.pdf"), width=8, height=6)


muller_Europebystate_raw2 = ggplot(data=data_agbyweekregion2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  facet_wrap(~ division) +
  # geom_col(aes(lwd=I(1.2), colour=NULL, fill=LINEAGE1), width=1, position="fill") +
  geom_area(aes(lwd=I(1.2), colour=NULL, fill=LINEAGE2, group=LINEAGE2), position="fill") +
  scale_fill_manual("", values=lineage_cols2) +
...
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_Europebystate_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots by state_raw data.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots by state_raw data.pdf"), width=10, height=6)
...
library(splines)
set.seed(1)
fit1_Europe_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM)+STATE, weights=count, data=data_agbyweekregion2, maxit=1000)
fit2_Europe_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM)*STATE, weights=count, data=data_agbyweekregion2, maxit=1000)
fit3_Europe_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2)+STATE, weights=count, data=data_agbyweekregion2, maxit=1000)
fit4_Europe_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2)*STATE, weights=count, data=data_agbyweekregion2, maxit=1000)
fit5_Europe_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=3)+STATE, weights=count, data=data_agbyweekregion2, maxit=1000)
fit6_Europe_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=3)*STATE, weights=count, data=data_agbyweekregion2, maxit=1000)
BIC(fit1_Europe_multi, fit2_Europe_multi, fit3_Europe_multi, fit4_Europe_multi, fit5_Europe_multi, fit6_Europe_multi) 
# fit4_Europe_multi fits best (lowest BIC)

# growth rate advantage compared to Delta (difference in growth rate per day) 
emtrEurope = emtrends(fit4_Europe_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_Europe = data.frame(confint(emtrEurope, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrEurope$contrasts)$p.value)
delta_r_Europe
...
# growth rate advantage compared to Delta (difference in growth rate per day) with simple model fit1_Europe_multi
emtrEurope1 = emtrends(fit1_Europe_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_Europe1 = data.frame(confint(emtrEurope1, 
                                 adjust="none")$contrasts, 
                         p.value=as.data.frame(emtrEurope1$contrasts, adjust="none")$p.value)
delta_r_Europe1
...
# # pairwise growth rates advantages for simple multinomial model fit1_Europe_multi
# emtrEurope_pairw1 = emtrends(fit1_Europe_multi, pairwise ~ LINEAGE2,  
#                     var="DATE_NUM",  mode="latent",
#                     at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
# delta_r_pairw_Europe1 = data.frame(confint(emtrEurope_pairw1, 
#                                   adjust="none")$contrasts, 
#                           p.value=as.data.frame(emtrEurope_pairw1$contrasts, adjust="none")$p.value)
# delta_r_pairw_Europe1[grepl("\\<B.1.1.7\\>|\\<B.1.617.2\\>|\\<B.1.351\\>|\\<P.1\\>|^AY.3\\>", delta_r_pairw_Europe1$contrast)&
#                    !grepl("\\<B.1.2\\>|\\<B.1.1.519\\>|\\<B.1.351\\>|\\<B.1.427\\>|^B.1.429\\>|^B.1.526\\>", delta_r_pairw_Europe1$contrast)
#                    ,]




# fitted prop of different LINEAGES in the Europe today
multinom_preds_today_avg = data.frame(emmeans(fit4_Europe_multi, ~ LINEAGE2|1,
...
lineages_bystate = data.frame(emmeans(fit4_Europe_multi,
                   ~ LINEAGE2,
                   by=c("DATE_NUM","STATE"),
                   at=list(DATE_NUM=today_num),  
                   mode="prob", df=NA))
...
deltaplEuropeY3_bystate = delta_bystate
...
# deltaplEuropeY3_bystate$prob = deltaplEuropeY3_bystate$prob + AY3_bystate$prob
# AY3_bystate = AY3_bystate[order(AY3_bystate$prob, decreasing=T),]
# AY3_bystate
# 
# 
# deltaplEuropeY3_bystate = deltaplEuropeY3_bystate[order(deltaplEuropeY3_bystate$prob, decreasing=T),]
# deltaplEuropeY3_bystate
...
fit_Europe_multi_preds_bystate4 = data.frame(predgrid, as.data.frame(predict(fit4_Europe_multi, newdata=predgrid, type="prob")), check.names=F)
fit_Europe_multi_preds_bystate1 = data.frame(predgrid, as.data.frame(predict(fit1_Europe_multi, newdata=predgrid, type="prob")), check.names=F)
fit_Europe_multi_preds_bystate = fit_Europe_multi_preds_bystate1
library(tidyr)
fit_Europe_multi_preds_bystate = gather(fit_Europe_multi_preds_bystate, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_Europe_multi_preds_bystate$collection_date = as.Date(fit_Europe_multi_preds_bystate$DATE_NUM, origin="1970-01-01")
fit_Europe_multi_preds_bystate$LINEAGE2 = factor(fit_Europe_multi_preds_bystate$LINEAGE2, levels=levels_LINEAGE2) 
# order states from highest to lowest incidence of delta+AY.3
levels_states2 = levels_states[match(deltaplEuropeY3_bystate$STATE,levels_states)]
fit_Europe_multi_preds_bystate$STATE = factor(fit_Europe_multi_preds_bystate$STATE, levels=levels_states2)
data_agbyweekregion2$division = factor(data_agbyweekregion2$division, levels=levels_states2)
data_agbyweekregion2$LINEAGE2 = factor(data_agbyweekregion2$LINEAGE2, levels=levels_LINEAGE2)

muller_Europe_mfit_bystate = ggplot(data=fit_Europe_multi_preds_bystate, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN THE Europe\n(GISAID data, multinomial fit)")
muller_Europe_mfit_bystate

ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots_multinom fit by state_fit1.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots_multinom fit by state.pdf"), width=10, height=6)
...
ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots by state_raw data_fit1.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots by state_raw data.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_Europebystate_raw2 + coord_cartesian(xlim=c(as.Date("2021-01-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_Europe_mfit_bystate+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots multipanel_multinom fit by state_fit1.png"), width=10, height=15)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_muller plots multipanel_multinom fit by state.pdf"), width=10, height=15)
...
fit_Europe_multi_preds_bystate_withCI4 = data.frame(emmeans(fit4_Europe_multi,
                                                        ~ LINEAGE2,
                                                        by=c("DATE_NUM","STATE"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                        mode="prob", df=NA))
fit_Europe_multi_preds_bystate_withCI1 = data.frame(emmeans(fit1_Europe_multi,
                                                         ~ LINEAGE2,
                                                         by=c("DATE_NUM","STATE"),
                                                         at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                         mode="prob", df=NA))
fit_Europe_multi_preds_bystate_withCI = fit_Europe_multi_preds_bystate_withCI1
fit_Europe_multi_preds_bystate_withCI$collection_date = as.Date(fit_Europe_multi_preds_bystate_withCI$DATE_NUM, origin="1970-01-01")
fit_Europe_multi_preds_bystate_withCI$LINEAGE2 = factor(fit_Europe_multi_preds_bystate_withCI$LINEAGE2, levels=levels_LINEAGE2)
fit_Europe_multi_preds_bystate_withCI$STATE = factor(fit_Europe_multi_preds_bystate_withCI$STATE, levels=levels_states2)
fit_Europe_multi_preds2 = fit_Europe_multi_preds_bystate_withCI

# on logit scale:

ymin = 0.001
ymax = 0.999
fit_Europe_multi_preds2$asymp.LCL[fit_Europe_multi_preds2$asymp.LCL<ymin] = ymin
fit_Europe_multi_preds2$asymp.UCL[fit_Europe_multi_preds2$asymp.UCL<ymin] = ymin
fit_Europe_multi_preds2$asymp.UCL[fit_Europe_multi_preds2$asymp.UCL>ymax] = ymax
fit_Europe_multi_preds2$prob[fit_Europe_multi_preds2$prob<ymin] = ymin

plot_Europe_mfit_logit = qplot(data=fit_Europe_multi_preds2, x=collection_date, y=prob, geom="blank") +
  facet_wrap(~ STATE) +
...
plot_Europe_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_multinom fit by state_logit scale_fit1.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_multinom fit by state_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_Europe_mfit = qplot(data=fit_Europe_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  facet_wrap(~ STATE) +
...
ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_multinom fit by state_response scale_fit1.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_multinom fit by state_response scale.pdf"), width=10, height=6)




# overall multinomial model predictions with confidence intervals
fit_Europe_multi_preds_withCI4 = data.frame(emmeans(fit4_Europe_multi,
                                                        ~ LINEAGE2,
                                                        by=c("DATE_NUM"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                        mode="prob", df=NA))
fit_Europe_multi_preds_withCI1 = data.frame(emmeans(fit1_Europe_multi,
                                                 ~ LINEAGE2,
                                                 by=c("DATE_NUM"),
                                                 at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                 mode="prob", df=NA))
fit_Europe_multi_preds_withCI = fit_Europe_multi_preds_withCI1
fit_Europe_multi_preds_withCI$collection_date = as.Date(fit_Europe_multi_preds_withCI$DATE_NUM, origin="1970-01-01")
fit_Europe_multi_preds_withCI$LINEAGE2 = factor(fit_Europe_multi_preds_withCI$LINEAGE2, levels=levels_LINEAGE2)
fit_Europe_multi_preds3 = fit_Europe_multi_preds_withCI

# on logit scale:

ymin = 0.001
ymax = 0.999
fit_Europe_multi_preds3$asymp.LCL[fit_Europe_multi_preds3$asymp.LCL<ymin] = ymin
fit_Europe_multi_preds3$asymp.UCL[fit_Europe_multi_preds3$asymp.UCL<ymin] = ymin
fit_Europe_multi_preds3$asymp.UCL[fit_Europe_multi_preds3$asymp.UCL>ymax] = ymax
fit_Europe_multi_preds3$prob[fit_Europe_multi_preds3$prob<ymin] = ymin

plot_Europe_avg_mfit_logit = qplot(data=fit_Europe_multi_preds3, x=collection_date, y=prob, geom="blank") +
  # facet_wrap(~ STATE) +
  geom_ribbon(aes(y=prob, ymin=asymp.LCL, ymax=asymp.UCL, colour=NULL,
                  fill=LINEAGE2
  ), alpha=I(0.3)) +
  ...
  xlab("Collection date")+
  coord_cartesian(xlim=c(as.Date("2021-01-01"),as.Date(date.to, origin="1970-01-01")), ylim=c(0.001, 0.9901), expand=c(0,0))
plot_Europe_avg_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_multinom fit_logit scale.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_multinom fit_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_Europe_avg_mfit = qplot(data=fit_Europe_multi_preds3, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ STATE) +
  geom_ribbon(aes(y=100*prob, ymin=100*asymp.LCL, ymax=100*asymp.UCL, colour=NULL,
                  fill=LINEAGE2
  ), alpha=I(0.3)) +
  ...
  theme(legend.position = "right") +
  xlab("Collection date")
plot_Europe_avg_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_multinom fit_response scale.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\Europe_multinom fit_response scale.pdf"), width=10, height=6)
...
cases_tot = as.data.frame(get_regional_data(country = "Europe", level="1"))
...
fit_Europe_multi_preds_bystate_withCI$totcases = cases_tot$cases_new[match(interaction(fit_Europe_multi_preds_bystate_withCI$DATE_NUM,fit_Europe_multi_preds_bystate_withCI$STATE),
                                                            interaction(cases_tot$DATE_NUM,cases_tot$STATE))]
fit_Europe_multi_preds_bystate_withCI$cases = fit_Europe_multi_preds_bystate_withCI$totcases * fit_Europe_multi_preds_bystate_withCI$prob
fit_Europe_multi_preds_bystate_withCI$cases[fit_Europe_multi_preds_bystate_withCI$cases<=0.001] = NA
cases_emmeans = as.data.frame(emmeans(fit_cases, ~ DATE_NUM|STATE, at=list(DATE_NUM=seq(date.from, date.to, by=0.5)
), type="response"))
fit_Europe_multi_preds_bystate_withCI$smoothed_totcases = cases_emmeans$rate[match(interaction(fit_Europe_multi_preds_bystate_withCI$DATE_NUM,fit_Europe_multi_preds_bystate_withCI$STATE),
                                                                    interaction(cases_emmeans$DATE_NUM,cases_emmeans$STATE))]
fit_Europe_multi_preds_bystate_withCI$smoothed_cases = fit_Europe_multi_preds_bystate_withCI$smoothed_totcases * fit_Europe_multi_preds_bystate_withCI$prob
fit_Europe_multi_preds_bystate_withCI$smoothed_cases[fit_Europe_multi_preds_bystate_withCI$smoothed_cases<=0.001] = NA



fit_Europe_multi_preds_bystate$totcases = cases_tot$cases_new[match(interaction(fit_Europe_multi_preds_bystate$DATE_NUM,fit_Europe_multi_preds_bystate$STATE),
                                                                        interaction(cases_tot$DATE_NUM,cases_tot$STATE))]
fit_Europe_multi_preds_bystate$cases = fit_Europe_multi_preds_bystate$totcases * fit_Europe_multi_preds_bystate$prob
fit_Europe_multi_preds_bystate$cases[fit_Europe_multi_preds_bystate$cases<=0.001] = NA
cases_emmeans = as.data.frame(emmeans(fit_cases, ~ DATE_NUM|STATE, at=list(DATE_NUM=seq(date.from, date.to, by=0.5)
), type="response"))
fit_Europe_multi_preds_bystate$smoothed_totcases = cases_emmeans$rate[match(interaction(fit_Europe_multi_preds_bystate$DATE_NUM,fit_Europe_multi_preds_bystate$STATE),
                                                                                interaction(cases_emmeans$DATE_NUM,cases_emmeans$STATE))]
fit_Europe_multi_preds_bystate$smoothed_cases = fit_Europe_multi_preds_bystate$smoothed_totcases * fit_Europe_multi_preds_bystate$prob
fit_Europe_multi_preds_bystate$smoothed_cases[fit_Europe_multi_preds_bystate$smoothed_cases<=0.001] = NA


ggplot(data=fit_Europe_multi_preds_bystate, 
  ...
  ggtitle("NEW CONFIRMED SARS-CoV2 CASES PER DAY BY VARIANT IN THE Europe\n(case data NYT & multinomial fit to GISAID data)") +
  scale_fill_manual("variant", values=lineage_cols2) +
  scale_colour_manual("variant", values=lineage_cols2) +
...
ggplot(data=fit_Europe_multi_preds_bystate, 
  ...
  facet_wrap(~ STATE, scale="free") +
  geom_area(data=fit_Europe_multi_preds_bystate[fit_Europe_multi_preds_bystate$LINEAGE2=="B.1.1.7",], aes(lwd=I(1.2), colour=NULL), fill=I("#0085FF"), position="identity", alpha=I(0.7)) +
  geom_area(data=fit_Europe_multi_preds_bystate[fit_Europe_multi_preds_bystate$LINEAGE2=="B.1.617.2",], aes(lwd=I(1.2), colour=NULL), fill=I("magenta"), position="identity", alpha=I(0.7)) +
  geom_area(data=fit_Europe_multi_preds_bystate[fit_Europe_multi_preds_bystate$LINEAGE2=="AY.3",], aes(lwd=I(1.2), colour=NULL), fill=muted(I("magenta")), position="identity", alpha=I(0.7)) +
  geom_area(data=fit_Europe_multi_preds_bystate[fit_Europe_multi_preds_bystate$LINEAGE2=="B.1.351",], aes(lwd=I(1.2), colour=NULL), fill="#9A9D00", position="identity", alpha=I(0.7)) +
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01")),
                     labels=substring(months(as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01"))),1,1),
                     limits=as.Date(c("2021-01-01",NA)), expand=c(0,0)) +
  scale_y_log10() +
...
qplot(data=avg_r_cases, x=DATE-7, y=Re, ymin=Re_LOWER, ymax=Re_UPPER, geom="ribbon", alpha=I(0.5), fill=I("steelblue")) +
  ...
  ggtitle("Re VALUES IN SELECTED STATES IN THE Europe\n(case data NYT)") +
  # labs(tag = tag) +
...
# for simple multinomial model fit1_Europe_multi
above_avg_r_variants1 = do.call(rbind, lapply(levels_states2, function(STATE) { do.call(rbind, 
                                                                                            lapply(seq(date.from,
                                                                                                       date.to, by=3), 
                                                                                                   function (d) { 
                                                                                                     wt = as.data.frame(emmeans(fit1_Europe_multi, ~ LINEAGE2 , by="STATE", 
                                                                                                     ...
                                                                                                     EMT = emtrends(fit1_Europe_multi,  ~ LINEAGE2 , by=c("DATE_NUM", "STATE"),
                                                                                                                    var="DATE_NUM", mode="latent",
...
above_avg_r_variants$prob = fit_Europe_multi_preds_bystate_withCI$prob[match(interaction(round(above_avg_r_variants$DATE_NUM),
                                                                          as.character(above_avg_r_variants$LINEAGE2),
                                                                          as.character(above_avg_r_variants$STATE)),
                                                              interaction(round(fit_Europe_multi_preds_bystate_withCI$DATE_NUM),
                                                                          as.character(fit_Europe_multi_preds_bystate_withCI$LINEAGE2),
                                                                          as.character(fit_Europe_multi_preds_bystate_withCI$STATE)))]
