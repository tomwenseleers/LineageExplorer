...
nrow(GISAID_sel) # 41488

sum(GISAID_sel$LINEAGE1=="B.1.617+") # 347
unique(GISAID_sel$country[GISAID_sel$LINEAGE1=="B.1.1.7"])
sum(GISAID_sel$LINEAGE1=="B.1.1.7") # 5923
...
main_lineages
# "AE.8"      "B.1"       "B.1.1.176" "B.1.1.7"   "B.1.2"     "B.1.279"   "B.1.36+"   "B.1.438.1" "P.1"  
...
table(GISAID_sel$LINEAGE1)
#   AE.8       B.1 B.1.1.176 B.1.1.519   B.1.1.7     B.1.2   B.1.279   B.1.351   B.1.36+ B.1.438.1   B.1.525  B.1.617+     other       P.1 
# 1324      2418      1308       478      5923      3379      1829       199      4383      5609      1124       347     11085      2082 
...
# "B.1.1.7"   "AE.8"      "B.1"       "B.1.1.176" "B.1.1.519" "B.1.2"     "B.1.279"   "B.1.351"   "B.1.36+"   "B.1.438.1" "B.1.525"   "B.1.617+"  "other"    
# "P.1"   
...
range(GISAID_sel$date) # "2020-01-01" "2021-05-24"
...
muller_canada_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN CANADA\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_canada_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_muller plots_raw data.png"), width=8, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_muller plots_raw data.pdf"), width=8, height=6)
...
library(splines)
set.seed(1)
fit1_canada_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_canada_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_canada_multi, fit2_canada_multi) 
#             df      BIC
# fit1_canada_multi 28 137118.5
# fit2_canada_multi 42 132440.6

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrcanada = emtrends(fit2_canada_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_canada = data.frame(confint(emtrcanada, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrcanada$contrasts)$p.value)
delta_r_canada
#                         contrast      estimate         SE df   asymp.LCL  asymp.UCL   p.value
# 1       AE.8 - B.1.1.7 -0.037649173 0.003109517 NA -0.043743715 -0.031554632 1.033507e-12
# 2        B.1 - B.1.1.7  0.027688173 0.002539123 NA  0.022711583  0.032664762 2.115752e-12
# 3  B.1.1.176 - B.1.1.7  0.008993846 0.002453681 NA  0.004184720  0.013802972 8.045521e-03
# 4  B.1.1.519 - B.1.1.7  0.002511734 0.005109485 NA -0.007502672  0.012526139 9.956353e-01
# 5      B.1.2 - B.1.1.7  0.003759525 0.002403805 NA -0.000951846  0.008470896 6.263013e-01
# 6    B.1.279 - B.1.1.7 -0.046377037 0.003974024 NA -0.054165980 -0.038588094 1.127543e-12
# 7    B.1.351 - B.1.1.7  0.032336326 0.004984235 NA  0.022567406  0.042105247 1.071963e-06
# 8  (B.1.36+) - B.1.1.7  0.008645265 0.002305786 NA  0.004126007  0.013164523 6.341603e-03
# 9  B.1.438.1 - B.1.1.7  0.029841862 0.002007292 NA  0.025907642  0.033776082 9.821033e-13
# 10   B.1.525 - B.1.1.7  0.018209114 0.003840557 NA  0.010681761  0.025736468 3.162853e-04
# 11       P.1 - B.1.1.7 -0.095767970 0.002008044 NA -0.099703664 -0.091832275 9.814372e-13
# 12 B.1.617.1 - B.1.1.7  0.146139994 0.038030272 NA  0.071602031  0.220677957 4.849792e-03
# 13 B.1.617.2 - B.1.1.7  0.156438450 0.008265788 NA  0.140237803  0.172639097 9.814372e-13
# 14     other - B.1.1.7  0.011552835 0.002286061 NA  0.007072238  0.016033433 1.170008e-04


# fitted prop of different LINEAGES in the canada today
# 99% [97%-100%] now estimated to be B.1.617.2 across all regions
multinom_preds_today_avg = data.frame(emmeans(fit2_canada_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
#    LINEAGE2         prob           SE df    asymp.LCL    asymp.UCL
# 1    B.1.1.7 2.283852e-03 9.137733e-04 NA  4.928898e-04 4.074815e-03
# 2       AE.8 2.360344e-08 1.280779e-08 NA -1.499373e-09 4.870626e-08
# 3        B.1 1.351638e-04 6.065630e-05 NA  1.627960e-05 2.540479e-04
# 4  B.1.1.176 3.526360e-05 1.537609e-05 NA  5.127014e-06 6.540019e-05
# 5  B.1.1.519 1.218098e-04 6.740884e-05 NA -1.030905e-05 2.539287e-04
# 6      B.1.2 1.983387e-04 8.281504e-05 NA  3.602421e-05 3.606532e-04
# 7    B.1.279 1.360480e-10 1.112551e-10 NA -8.200791e-11 3.541039e-10
# 8    B.1.351 4.742840e-04 2.630833e-04 NA -4.134980e-05 9.899179e-04
# 9    B.1.36+ 2.684486e-04 1.105484e-04 NA  5.177773e-05 4.851195e-04
# 10 B.1.438.1 1.029410e-02 4.046398e-03 NA  2.363301e-03 1.822489e-02
# 11   B.1.525 2.272145e-03 1.007816e-03 NA  2.968609e-04 4.247429e-03
# 12       P.1 7.059856e-06 2.981679e-06 NA  1.215873e-06 1.290384e-05
# 13 B.1.617.1 2.011638e-02 3.600134e-02 NA -5.044495e-02 9.067771e-02
# 14 B.1.617.2 9.633802e-01 3.691419e-02 NA  8.910297e-01 1.035731e+00
# 15     other 4.129527e-04 1.692517e-04 NA  8.122546e-05 7.446799e-04

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9977161 0.8433849 1.1520474 
...
fit_canada_multi_preds = data.frame(predgrid, as.data.frame(predict(fit2_canada_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_canada_multi_preds = gather(fit_canada_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_canada_multi_preds$collection_date = as.Date(fit_canada_multi_preds$DATE_NUM, origin="1970-01-01")
fit_canada_multi_preds$LINEAGE2 = factor(fit_canada_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 

muller_canada_mfit = ggplot(data=fit_canada_multi_preds, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN CANADA\n(GISAID data, multinomial fit)")
muller_canada_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_muller plots_multinom fit.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_muller plots_multinom fit.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_canada_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_canada_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_muller plots multipanel_multinom fit.png"), width=10, height=10)
ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_muller plots multipanel_multinom fit.pdf"), width=10, height=10)
...
fit_canada_multi_preds_bystate_withCI = data.frame(emmeans(fit3_canada_multi,
                                                        ~ LINEAGE2,
                                                        by=c("DATE_NUM","STATE"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                        mode="prob", df=NA))
fit_canada_multi_preds_bystate_withCI$collection_date = as.Date(fit_canada_multi_preds_bystate_withCI$DATE_NUM, origin="1970-01-01")
fit_canada_multi_preds_bystate_withCI$LINEAGE2 = factor(fit_canada_multi_preds_bystate_withCI$LINEAGE2, levels=levels_LINEAGE2)
fit_canada_multi_preds2 = fit_canada_multi_preds_bystate_withCI

fit_canada_multi_preds_bystate_withCI[fit_canada_multi_preds_bystate_withCI$collection_date==as.Date("2021-06-21")&fit_canada_multi_preds_bystate_withCI$LINEAGE2=="B.1.617.2",]
...
# fit_canada_multi_preds2 = fit_canada_multi_preds_bystate # without CIs
# fit_canada_multi_preds2$asymp.LCL = NA
# fit_canada_multi_preds2$asymp.UCL = NA
...
ymin = 0.001
ymax = 0.999
fit_canada_multi_preds2$asymp.LCL[fit_canada_multi_preds2$asymp.LCL<ymin] = ymin
fit_canada_multi_preds2$asymp.UCL[fit_canada_multi_preds2$asymp.UCL<ymin] = ymin
fit_canada_multi_preds2$asymp.UCL[fit_canada_multi_preds2$asymp.UCL>ymax] = ymax
fit_canada_multi_preds2$prob[fit_canada_multi_preds2$prob<ymin] = ymin

plot_canada_mfit_logit = qplot(data=fit_canada_multi_preds2, x=collection_date, y=prob, geom="blank") +
  facet_wrap(~ STATE) +
  ...
  ylab("Share (%)") +
  theme_hc() + xlab("") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN THE canada\n(GISAID data, multinomial fit)") +
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01")),
                     labels=substring(months(as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01"))),1,1),
                     limits=as.Date(c("2020-11-01",NA)), expand=c(0,0)) +
...
plot_canada_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_multinom fit by state_logit scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_multinom fit by state_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_canada_mfit = qplot(data=fit_canada_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  facet_wrap(~ STATE) +
...
ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_multinom fit by state_response scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_multinom fit by state_response scale.pdf"), width=10, height=6)




# overall multinomial model predictions with confidence intervals
fit_canada_multi_preds_withCI = data.frame(emmeans(fit3_canada_multi,
                                                        ~ LINEAGE2,
                                                        by=c("DATE_NUM"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=7)),  # by=7 to speed up things a bit
                                                        mode="prob", df=NA))
fit_canada_multi_preds_withCI$collection_date = as.Date(fit_canada_multi_preds_withCI$DATE_NUM, origin="1970-01-01")
fit_canada_multi_preds_withCI$LINEAGE2 = factor(fit_canada_multi_preds_withCI$LINEAGE2, levels=levels_LINEAGE2)
fit_canada_multi_preds3 = fit_canada_multi_preds_withCI

fit_canada_multi_preds_withCI[fit_canada_multi_preds_withCI$collection_date==as.Date("2021-06-21")&fit_canada_multi_preds_withCI$LINEAGE2=="B.1.617.2",]
#        LINEAGE2 DATE_NUM         STATE       prob          SE df  asymp.LCL  asymp.UCL collection_date
# 615 B.1.617.2    18799 0.4215232 0.01522866 NA 0.3916756 0.4513708      2021-06-21

# fit_canada_multi_preds2 = fit_canada_multi_preds # without CIs
# fit_canada_multi_preds2$asymp.LCL = NA
# fit_canada_multi_preds2$asymp.UCL = NA
...
ymin = 0.001
ymax = 0.999
fit_canada_multi_preds3$asymp.LCL[fit_canada_multi_preds3$asymp.LCL<ymin] = ymin
fit_canada_multi_preds3$asymp.UCL[fit_canada_multi_preds3$asymp.UCL<ymin] = ymin
fit_canada_multi_preds3$asymp.UCL[fit_canada_multi_preds3$asymp.UCL>ymax] = ymax
fit_canada_multi_preds3$prob[fit_canada_multi_preds3$prob<ymin] = ymin

plot_canada_avg_mfit_logit = qplot(data=fit_canada_multi_preds3, x=collection_date, y=prob, geom="blank") +
  # facet_wrap(~ STATE) +
  geom_ribbon(aes(y=prob, ymin=asymp.LCL, ymax=asymp.UCL, colour=NULL,
                  fill=LINEAGE2
  ), alpha=I(0.3)) +
  ...
  xlab("Collection date")+
  coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")), ylim=c(0.001, 0.9901), expand=c(0,0))
plot_canada_avg_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_multinom fit_logit scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_multinom fit_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_canada_avg_mfit = qplot(data=fit_canada_multi_preds3, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ STATE) +
  geom_ribbon(aes(y=100*prob, ymin=100*asymp.LCL, ymax=100*asymp.UCL, colour=NULL,
                  fill=LINEAGE2
  ), alpha=I(0.3)) +
  ...
  theme(legend.position = "right") +
  xlab("Collection date")
plot_canada_avg_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_multinom fit_response scale.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\canada_multinom fit_response scale.pdf"), width=10, height=6)
