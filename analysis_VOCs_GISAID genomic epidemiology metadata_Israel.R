...
range(GISAID_sel$date) # "2020-02-23" "2021-05-11"
...
muller_israel_raw2 = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN ISRAEL\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_israel_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\israel_muller plots_raw data.png"), width=8, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\israel_muller plots_raw data.pdf"), width=8, height=6)
...
library(splines)
set.seed(1)
fit1_israel_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=1000)
fit2_israel_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=1000)
BIC(fit1_israel_multi, fit2_israel_multi) 
#             df      BIC
# fit1_israel_multi 20 23906.88
# fit2_israel_multi 30 23387.05

# growth rate advantage compared to UK type B.1.1.7 (difference in growth rate per day) 
emtrisrael = emtrends(fit2_israel_multi, trt.vs.ctrl ~ LINEAGE2,  
                   var="DATE_NUM",  mode="latent",
                   at=list(DATE_NUM=max(GISAID_sel$DATE_NUM)))
delta_r_israel = data.frame(confint(emtrisrael, 
                                 adjust="none", df=NA)$contrasts, 
                         p.value=as.data.frame(emtrisrael$contrasts)$p.value)
delta_r_israel
# contrast     estimate           SE df    asymp.LCL     asymp.UCL      p.value
# 1        B.1 - B.1.1.7 -0.001283067 4.104568e-03 NA -0.009327874  0.006761739 9.980272e-01
# 2   B.1.1.50 - B.1.1.7 -0.030323124 4.012226e-03 NA -0.038186942 -0.022459306 1.929667e-07
# 3  B.1.1.519 - B.1.1.7 -0.352536413 4.267405e-02 NA -0.436176011 -0.268896814 3.112743e-08
# 4    B.1.351 - B.1.1.7 -0.075795846 1.201475e-02 NA -0.099344321 -0.052247371 5.628166e-06
# 5    B.1.362 - B.1.1.7 -0.039569715 4.864905e-03 NA -0.049104754 -0.030034675 4.312507e-08
# 6       None - B.1.1.7  0.003410190 3.940632e-03 NA -0.004313307  0.011133687 9.158294e-01
# 7        P.1 - B.1.1.7  0.060083503 1.846469e-02 NA  0.023893381  0.096273624 2.246385e-02
# 8  B.1.617.1 - B.1.1.7  0.101504623 1.694280e-12 NA  0.101504623  0.101504623 4.152234e-14
# 9  B.1.617.2 - B.1.1.7  0.036561747 9.108525e-03 NA  0.018709365  0.054414128 3.195246e-03
# 10     other - B.1.1.7 -0.001805415 3.176711e-03 NA -0.008031654  0.004420824 9.807907e-01


# fitted prop of different LINEAGES in the israel today
# 93% [83%-100%] now estimated to be B.1.617.2 across all regions
multinom_preds_today_avg = data.frame(emmeans(fit2_israel_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1    B.1.1.7 5.291184e-01 1.235668e-01 NA  2.869319e-01 7.713049e-01
# 2        B.1 2.272474e-03 1.214916e-03 NA -1.087172e-04 4.653666e-03
# 3   B.1.1.50 3.172945e-04 1.586572e-04 NA  6.332026e-06 6.282569e-04
# 4  B.1.1.519 1.070143e-21 6.592710e-21 NA -1.185133e-20 1.399162e-20
# 5    B.1.351 5.252001e-06 6.506229e-06 NA -7.499973e-06 1.800397e-05
# 6    B.1.362 2.558289e-05 1.656935e-05 NA -6.892437e-06 5.805822e-05
# 7       None 2.658163e-02 1.102745e-02 NA  4.968232e-03 4.819503e-02
# 8        P.1 1.314315e-01 1.375514e-01 NA -1.381643e-01 4.010272e-01
# 9  B.1.617.1 2.306818e-11 5.387189e-12 NA  1.250948e-11 3.362687e-11
# 10 B.1.617.2 2.971267e-01 1.261478e-01 NA  4.988162e-02 5.443717e-01
# 11     other 1.312123e-02 5.089165e-03 NA  3.146652e-03 2.309581e-02

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.47088161 -0.08028458  1.02204780  
...
fit_israel_multi_preds = data.frame(predgrid, as.data.frame(predict(fit2_israel_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_israel_multi_preds = gather(fit_israel_multi_preds, LINEAGE2, prob, all_of(levels_LINEAGE2), factor_key=TRUE)
fit_israel_multi_preds$collection_date = as.Date(fit_israel_multi_preds$DATE_NUM, origin="1970-01-01")
fit_israel_multi_preds$LINEAGE2 = factor(fit_israel_multi_preds$LINEAGE2, levels=levels_LINEAGE2) 

muller_israel_mfit = ggplot(data=fit_israel_multi_preds, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN ISRAEL\n(GISAID data, multinomial fit)")
muller_israel_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\israel_muller plots_multinom fit.png"), width=10, height=6)
ggsave(file=paste0(".\\plots\\",plotdir,"\\israel_muller plots_multinom fit.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_israel_raw2 + coord_cartesian(xlim=c(as.Date("2020-11-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_israel_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\israel_muller plots multipanel_multinom fit.png"), width=10, height=10)
ggsave(file=paste0(".\\plots\\",plotdir,"\\israel_muller plots multipanel_multinom fit.pdf"), width=10, height=10)
