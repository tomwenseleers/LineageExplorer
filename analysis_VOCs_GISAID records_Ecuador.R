...
d1 = read_tsv(file(".//data//GISAID//Ecuador//gisaid_hcov-19_2021_07_15_20.tsv"), col_types = cols(.default = "c")) 
...
range(GISAID$date) # "2020-02-25" "2021-07-04"
...
# ANALYSIS OF VOCs IN ECUADOR ####

# sel_countries = "Ecuador"
# GISAID[GISAID$country %in% sel_countries,]

GISAID_sel = GISAID

sum(GISAID_sel$LINEAGE=="B.1.617.2") # 9
...
remove = remove[!(remove %in% c("B.1.351","B.1.1.7","P.1","B.1.617.2","B.1.617.1","B.1.1.519"))]
GISAID_sel$LINEAGE[(GISAID_sel$LINEAGE %in% remove)] = "other" # minority VOCs
table(GISAID_sel$LINEAGE)
# B.1     B.1.1 B.1.1.519   B.1.1.7   B.1.526 B.1.617.2      C.37     other       P.1 
# 71       123         1       137       207         9        67       206        53 
...
#  "B.1.1.7"   "B.1"       "B.1.1"     "B.1.1.519" "B.1.526"   "B.1.617.2" "C.37"      "other"     "P.1"  
levels_LINEAGE = c("B.1.1.7","B.1","B.1.1","B.1.1.519","B.1.526",
                    "P.1","B.1.617.2","other")
...
sum(data_agbyday[data_agbyday$LINEAGE=="B.1.617.1","total"]) == nrow(GISAID_sel) # correct
...
sum(data_agbyweek[data_agbyweek$LINEAGE=="B.1.617.1","total"]) == nrow(GISAID_sel) # correct
...
lineage_cols2[which(levels(GISAID_sel$LINEAGE)=="B.1.617.2")] = "magenta"
lineage_cols2[which(levels(GISAID_sel$LINEAGE)=="other")] = "grey75"

muller_ecuador_raw2 = ggplot(data=data_agbyweek, aes(x=collection_date, y=count, group=LINEAGE)) + 
  ...
  labs(title = "SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN ECUADOR\n(GISAID data)") 
# +
# coord_cartesian(xlim=c(1,max(GISAID_sel$Week)))
muller_ecuador_raw2

ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_muller plots_raw data.png"), width=8, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_muller plots_raw data.pdf"), width=8, height=6)


# multinomial fits
data_agbyweek$LINEAGE = relevel(data_agbyweek$LINEAGE, ref="B.1.1.7")
data_agbyweek$DATE_NUM = as.numeric(data_agbyweek$collection_date)

library(nnet)
library(splines)
set.seed(1)
fit1_ecuador_multi = nnet::multinom(LINEAGE ~ scale(DATE_NUM), weights=count, data=data_agbyweek, maxit=1000)
fit2_ecuador_multi = nnet::multinom(LINEAGE ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek, maxit=1000)
...
# df      BIC
# fit1_ecuador_multi 14 2471.378
# fit2_ecuador_multi 21 2465.753
# fit3_ecuador_multi 28 2487.492
...
# 1       B.1 - B.1.1.7  0.0094180975 0.006170671 NA -0.002676195 0.02151239 0.50108150
# 2     B.1.1 - B.1.1.7  0.0007490536 0.006198380 NA -0.011399548 0.01289766 0.99972025
# 3 B.1.1.519 - B.1.1.7 -0.0228687412 0.054550147 NA -0.129785065 0.08404758 0.98467763
# 4   B.1.526 - B.1.1.7  0.0140298158 0.006155562 NA  0.001965136 0.02609450 0.15897497
# 5       P.1 - B.1.1.7  0.0157162086 0.009682407 NA -0.003260961 0.03469338 0.44458546
# 6 B.1.617.2 - B.1.1.7  0.3371900529 0.073893999 NA  0.192360477 0.48201963 0.00105834
# 7     other - B.1.1.7  0.0155216360 0.005791225 NA  0.004171044 0.02687223 0.07347013


# fitted prop of different LINEAGES in the ecuador today
multinom_preds_today_avg = data.frame(emmeans(fit2_ecuador_multi, ~ LINEAGE|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_preds_today_avg
# LINEAGE         prob           SE df     asymp.LCL    asymp.UCL
# 1   B.1.1.7 5.768152e-04 9.523989e-04 NA -1.289852e-03 2.443483e-03
# 2       B.1 2.327083e-04 3.858554e-04 NA -5.235545e-04 9.889710e-04
# 3     B.1.1 1.030195e-04 1.716839e-04 NA -2.334748e-04 4.395137e-04
# 4 B.1.1.519 7.874172e-07 3.328940e-06 NA -5.737185e-06 7.312019e-06
# 5   B.1.526 2.043508e-03 3.344202e-03 NA -4.511008e-03 8.598023e-03
# 6       P.1 5.831649e-04 9.781111e-04 NA -1.333898e-03 2.500227e-03
# 7 B.1.617.2 9.952354e-01 7.761977e-03 NA  9.800222e-01 1.010449e+00
# 8     other 1.224571e-03 2.008549e-03 NA -2.712113e-03 5.161254e-03

# % non-B.1.1.7
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#      prob asymp.LCL asymp.UCL 
# 0.9994232 0.9707024 1.0281439
...
fit_ecuador_multi_preds = data.frame(predgrid, as.data.frame(predict(fit2_ecuador_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
fit_ecuador_multi_preds = gather(fit_ecuador_multi_preds, LINEAGE, prob, all_of(levels_LINEAGE), factor_key=TRUE)
fit_ecuador_multi_preds$collection_date = as.Date(fit_ecuador_multi_preds$DATE_NUM, origin="1970-01-01")
fit_ecuador_multi_preds$LINEAGE = factor(fit_ecuador_multi_preds$LINEAGE, levels=levels_LINEAGE) 

muller_ecuador_mfit = ggplot(data=fit_ecuador_multi_preds, 
  ...
  theme_hc() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
  ylab("Share") +
  ggtitle("SPREAD OF SARS-CoV2 VARIANTS OF CONCERN IN ECUADOR\n(GISAID data, multinomial fit)")
muller_ecuador_mfit

ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_muller plots_multinom fit.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_muller plots_multinom fit.pdf"), width=10, height=6)


library(ggpubr)
ggarrange(muller_ecuador_raw2 + coord_cartesian(xlim=c(as.Date("2021-01-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
                   fill = guide_legend(override.aes = list(alpha = 0))), 
          muller_ecuador_mfit+ggtitle("Multinomial fit"), ncol=1)

ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_muller plots multipanel_multinom fit.png"), width=10, height=10)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_muller plots multipanel_multinom fit.pdf"), width=10, height=10)
...
fit_ecuador_multi_preds_withCI = data.frame(emmeans(fit2_ecuador_multi,
                                                        ~ LINEAGE,
                                                        by=c("DATE_NUM"),
                                                        at=list(DATE_NUM=seq(date.from, date.to, by=1)),  # by=XX to speed up things a bit
                                                        mode="prob", df=NA))
fit_ecuador_multi_preds_withCI$collection_date = as.Date(fit_ecuador_multi_preds_withCI$DATE_NUM, origin="1970-01-01")
fit_ecuador_multi_preds_withCI$LINEAGE = factor(fit_ecuador_multi_preds_withCI$LINEAGE, levels=levels_LINEAGE)
fit_ecuador_multi_preds2 = fit_ecuador_multi_preds_withCI
...
ymin = 0.001
ymax = 0.999
fit_ecuador_multi_preds2$asymp.LCL[fit_ecuador_multi_preds2$asymp.LCL<ymin] = ymin
fit_ecuador_multi_preds2$asymp.UCL[fit_ecuador_multi_preds2$asymp.UCL<ymin] = ymin
fit_ecuador_multi_preds2$asymp.UCL[fit_ecuador_multi_preds2$asymp.UCL>ymax] = ymax
fit_ecuador_multi_preds2$prob[fit_ecuador_multi_preds2$prob<ymin] = ymin

plot_ecuador_mfit_logit = qplot(data=fit_ecuador_multi_preds2, x=collection_date, y=prob, geom="blank") +
  # facet_wrap(~ STATE) +
...
plot_ecuador_mfit_logit

ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_multinom fit_logit scale.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_multinom fit_logit scale.pdf"), width=10, height=6)


# on response scale:
plot_ecuador_mfit = qplot(data=fit_ecuador_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ STATE) +
...
ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_multinom fit_response scale.png"), width=10, height=6)
# ggsave(file=paste0(".\\plots\\",plotdir,"\\ecuador_multinom fit_response scale.pdf"), width=10, height=6)
...
cases_tot = as.data.frame(get_national_data(countries = "Ecuador"))
...
fit_ecuador_multi_preds_withCI$totcases = cases_tot$cases_new[match(round(fit_ecuador_multi_preds_withCI$DATE_NUM),cases_tot$DATE_NUM)]
fit_ecuador_multi_preds_withCI$cases = fit_ecuador_multi_preds_withCI$totcases * fit_ecuador_multi_preds_withCI$prob
fit_ecuador_multi_preds_withCI$cases[fit_ecuador_multi_preds_withCI$cases<=0.001] = NA
...
fit_ecuador_multi_preds_withCI$smoothed_totcases = cases_emmeans$rate[match(fit_ecuador_multi_preds_withCI$DATE_NUM,cases_emmeans$DATE_NUM)]
fit_ecuador_multi_preds_withCI$smoothed_cases = fit_ecuador_multi_preds_withCI$smoothed_totcases * fit_ecuador_multi_preds_withCI$prob
fit_ecuador_multi_preds_withCI$smoothed_cases[fit_ecuador_multi_preds_withCI$smoothed_cases<=0.001] = NA
fit_ecuador_multi_preds_withCI$LINEAGE = factor(fit_ecuador_multi_preds_withCI$LINEAGE, levels=levels_LINEAGE)

ggplot(data=fit_ecuador_multi_preds_withCI[fit_ecuador_multi_preds_withCI$collection_date>=as.Date("2021-01-01"),], 
...
ggplot(data=fit_ecuador_multi_preds_withCI[fit_ecuador_multi_preds_withCI$collection_date>=as.Date("2021-01-01")&
                                              fit_ecuador_multi_preds_withCI$collection_date<=max(cases_tot$date),], 
...
above_avg_r_variants0 = do.call(rbind, lapply(seq(date.from,
                                                  date.to), 
                                              function (d) { 
                                                wt = as.data.frame(emmeans(fit2_ecuador_multi, ~ LINEAGE , at=list(DATE_NUM=d), type="response"))$prob   # important: these should sum to 1
                                                ...
                                                EMT = emtrends(fit2_ecuador_multi,  ~ LINEAGE , by=c("DATE_NUM"),
                                                               var="DATE_NUM", mode="latent",
                                                               at=list(DATE_NUM=d))
...
above_avg_r_variants$prob = fit_ecuador_multi_preds_withCI$prob[match(interaction(above_avg_r_variants$DATE_NUM,
